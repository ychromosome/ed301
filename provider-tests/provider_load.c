...
static int one_cold_load_cycle(void)
{
    ...
    if (libctx == NULL)
        return 0;
    v1 = ed301v2_load(libctx, &deflt);
    if (v1 == NULL)
        goto done;
    fetched = EVP_SIGNATURE_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
    if (fetched == NULL)
        goto done;
...
static int provider_lifetime_cycle(void)
{
    ...
    unsigned char value[ED301V2_SIG_BYTES];
    int ok = 0;

    if (libctx == NULL)
        goto done;
    first = ed301v2_load(libctx, &deflt);
    second = OSSL_PROVIDER_load(libctx, ED301V2_PROVIDER);
    signature = EVP_SIGNATURE_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
    keymgmt = EVP_KEYMGMT_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
    pkey = ed301v2_key_from_seed(libctx, POSITIVE_CASES[0].seed);
    ...
    if (strcmp(OSSL_PROVIDER_get0_name(signature_provider), ED301V2_PROVIDER) != 0
            ...
            || memcmp(value, POSITIVE_CASES[0].signature,
                ED301V2_SIG_BYTES) != 0
            || !ed301v2_digest_verify(libctx, pkey,
                POSITIVE_CASES[0].message,
...
int main(void)
{
    ...
    /* Fetch by name and property in an isolated context. */
    {
        ...
        ED301V2_CHECK(libctx != NULL, "library context");
        v1 = ed301v2_load(libctx, &deflt);
        ED301V2_CHECK(v1 != NULL, "provider load in isolated context");
        ED301V2_CHECK(!ed301v2_provider_has_dispatch(
                v1, OSSL_FUNC_PROVIDER_GET_CAPABILITIES),
            "ordinary provider has no TLS capability dispatch");
        ED301V2_CHECK(OSSL_PROVIDER_available(libctx, ED301V2_PROVIDER) == 1,
            "provider availability");

        sig_alg = EVP_SIGNATURE_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
        ED301V2_CHECK(sig_alg != NULL, "signature fetch by name and property");
        keymgmt = EVP_KEYMGMT_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
        ED301V2_CHECK(keymgmt != NULL, "keymgmt fetch by name and property");
        by_oid = EVP_SIGNATURE_fetch(libctx, ED301V2_OID_TEXT, ED301V2_PROP);
        ED301V2_CHECK(by_oid == NULL,
            "ordinary provider exposes no OID alias");
        ERR_clear_error();
        encoder = OSSL_ENCODER_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
        decoder = OSSL_DECODER_fetch(libctx, ED301V2_ALG, ED301V2_PROP);
        ED301V2_CHECK(encoder == NULL && decoder == NULL,
            "ordinary provider exposes no PKI codec operations");
        ED301V2_CHECK(ed301v2_registry_identity_state(&registry_nid)
                == ED301V2_REGISTRY_FREE
                && ed301v2_registry_sigid_state(registry_nid)
                    == ED301V2_REGISTRY_FREE,
         ...
         */
        {
            ...
            ED301V2_CHECK(pkey != NULL, "key before unload");
            OSSL_PROVIDER_unload(v1);
            OSSL_PROVIDER_unload(deflt);
    ...
    /* Without the provider, fetch must fail. */
    {
        ...
        EVP_SIGNATURE *missing =
            EVP_SIGNATURE_fetch(libctx, ED301V2_ALG, NULL);

        ED301V2_CHECK(deflt != NULL, "default provider");
        ED301V2_CHECK(missing == NULL,
    ...
    /* Independent library contexts do not interfere. */
    {
        ...
        OSSL_PROVIDER *first_v1 = ed301v2_load(first, &first_def);
        OSSL_PROVIDER *second_v1 = ed301v2_load(second, &second_def);
        EVP_SIGNATURE *first_alg =
            EVP_SIGNATURE_fetch(first, ED301V2_ALG, ED301V2_PROP);
        EVP_SIGNATURE *second_alg =
            EVP_SIGNATURE_fetch(second, ED301V2_ALG, ED301V2_PROP);

        ED301V2_CHECK(first_v1 != NULL && second_v1 != NULL
    ...
    /* Repeated cold load. */
    {
        ...
        ED301V2_CHECK(ok, "repeated cold load x%d (failed round %d)",
            REPEAT_ROUNDS, round);
    }
    ...
    /* Parallel cold load through a start gate. */
    {
        ...
        ED301V2_CHECK(ok, "parallel cold load (%d threads x %d rounds)",
            PARALLEL_THREADS, PARALLEL_ROUNDS);
    }

    return ed301v2_summary("provider_load");
}
