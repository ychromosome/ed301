 ...
 * one private library context, including context-bound X509/CSR/CRL
 * allocations and DER/PEM reparsing. No v2 provider is loaded globally.
 */
...
static OSSL_LIB_CTX *pki_libctx;
...
static X509_NAME *make_name(const char *common_name)
{
    ...
    if (name == NULL
            || X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC,
                (const unsigned char *)"Ed301-EdDSA-v2 experiment "
                    "(test-only)", -1, -1, 0) != 1
...
static X509 *make_cert_with_digest(
    ...
    const EVP_MD *digest)
{
    X509 *cert = X509_new_ex(pki_libctx, NULL);
...
static int algor_negative_controls(void)
{
    ...
    ASN1_OBJECT *target = OBJ_txt2obj(ED301V2_OID_TEXT, 1);
    ...
    if (ok) {
        ...
        target = NULL;
        ok = !ed301v2_pki_algorithm_is_exact(wrong)
            && !ed301v2_pki_algorithm_is_exact(with_null)
            && !ed301v2_pki_algorithm_is_exact(missing);
    }
...
static X509_REQ *reparse_request_der(const X509_REQ *request)
{
    ...
    if (length > 0 && der != NULL) {
        cursor = der;
        parsed = X509_REQ_new_ex(pki_libctx, NULL);
        if (parsed == NULL || d2i_X509_REQ(&parsed, &cursor, length) == NULL
                || cursor != der + length) {
...
static X509_REQ *reparse_request_pem(const X509_REQ *request)
{
    ...
    if (bio != NULL && request != NULL
        ...
        parsed = X509_REQ_new_ex(pki_libctx, NULL);
        if (parsed != NULL
                && PEM_read_bio_X509_REQ(bio, &parsed, NULL, NULL) == NULL) {
            X509_REQ_free(parsed);
            parsed = NULL;
        }
    }
...
static X509 *reparse_certificate_der(const X509 *certificate)
{
    ...
    if (length > 0 && der != NULL) {
        cursor = der;
        parsed = X509_new_ex(pki_libctx, NULL);
        if (parsed == NULL || d2i_X509(&parsed, &cursor, length) == NULL
                || cursor != der + length) {
            X509_free(parsed);
            parsed = NULL;
        }
...
static X509 *reparse_certificate_pem(const X509 *certificate)
{
    ...
    if (bio != NULL && certificate != NULL
        ...
        parsed = X509_new_ex(pki_libctx, NULL);
        if (parsed != NULL
                && PEM_read_bio_X509(bio, &parsed, NULL, NULL) == NULL) {
            X509_free(parsed);
            parsed = NULL;
        }
    }
...
static EVP_PKEY *make_ec_key(void)
{
    EVP_PKEY_CTX *context = EVP_PKEY_CTX_new_from_name(
        pki_libctx, "EC", "provider=default");
    EVP_PKEY *key = NULL;
...
static int verify_direct_chain(X509 *trust_anchor, X509 *leaf)
{
    ...
    X509_STORE_CTX *context = X509_STORE_CTX_new_ex(pki_libctx, NULL);
...
{
    X509_CRL *crl = X509_CRL_new_ex(pki_libctx, NULL);
...
int main(void)
{
    ...
    pki_libctx = OSSL_LIB_CTX_new();
    if (pki_libctx == NULL)
        return 1;
    OSSL_PROVIDER *deflt = NULL;
    OSSL_PROVIDER *v1;
    ed301v2_property = ED301V2_PKI_PROP;
    v1 = ed301v2_load_named(pki_libctx, &deflt, ED301V2_PKI_PROVIDER);
    EVP_PKEY *ca_key = ed301v2_keygen(pki_libctx);
    EVP_PKEY *intermediate_key = ed301v2_keygen(pki_libctx);
    EVP_PKEY *leaf_key = ed301v2_keygen(pki_libctx);

    ED301V2_CHECK(deflt != NULL && v1 != NULL,
        "providers in the private context");
    ED301V2_CHECK(ca_key != NULL && intermediate_key != NULL
     ...
     */
    {
        ...
        ED301V2_CHECK(nid != NID_undef
                && OBJ_obj2txt(oid_text, sizeof(oid_text),
                    OBJ_nid2obj(nid), 1) == (int)strlen(ED301V2_OID_TEXT)
                && strcmp(oid_text, ED301V2_OID_TEXT) == 0,
            "D5 project OID is registered byte-exactly");
        ED301V2_CHECK(OBJ_find_sigid_by_algs(&signature_nid, NID_undef,
                nid) == 1 && signature_nid == nid,
            "sigid maps the algorithm to itself with no digest");
        ED301V2_CHECK(algor_negative_controls(),
     ...
     */
    {
        X509_REQ *req = X509_REQ_new_ex(pki_libctx, NULL);
        X509_NAME *subject = make_name("Ed301-EdDSA-v2 CSR (test-only)");

        ED301V2_CHECK(req != NULL && subject != NULL
        ...
        ED301V2_CHECK(req != NULL && ed301v2_pki_verify_request(req, leaf_key),
        ...
        if (req != NULL) {
            ...
            ED301V2_CHECK(der_copy != NULL
                    && ed301v2_pki_verify_request(der_copy, leaf_key),
                "CSR DER reparse round trip preserves the exact profile");
            ED301V2_CHECK(pem_copy != NULL
                    && ed301v2_pki_verify_request(pem_copy, leaf_key),
        ...
        if (req != NULL) {
            ...
            ED301V2_CHECK(signature != NULL
                    && ed301v2_bit_string_length(signature) == 76,
                "CSR carries a 76-byte Ed301-EdDSA-v2 signature");
            if (algorithm != NULL) {
                char oid_text[96] = { 0 };

                OBJ_obj2txt(oid_text, sizeof(oid_text),
                    algorithm->algorithm, 1);
                ED301V2_CHECK(strcmp(oid_text, ED301V2_OID_TEXT) == 0,
                    "CSR AlgorithmIdentifier is the ephemeral test OID "
                    "(%s)", oid_text);
                ED301V2_CHECK(algorithm->parameter == NULL,
                    "CSR AlgorithmIdentifier is parameterless");
            }
            if (signature != NULL
                    && ed301v2_bit_string_length(signature) > 0) {
                /* Corrupt one signature byte in place. */
                unsigned char *bytes =
                    (unsigned char *)ASN1_STRING_get0_data(signature);

                bytes[0] ^= 1;
                ED301V2_CHECK(!ed301v2_pki_verify_request(req, leaf_key),
             ...
             */
            {
                X509_REQ *mutated = reparse_request_der(req);
                ...
                if (mutated != NULL) {
                    ...
                    if (mutated_outer != NULL)
                        X509_ALGOR_set0((X509_ALGOR *)mutated_outer,
                            OBJ_txt2obj(ED301V2_OID_TEXT, 1),
                            V_ASN1_NULL, NULL);
                    ordinary_verify = X509_REQ_verify(mutated, leaf_key);
                }
                ED301V2_CHECK(mutated != NULL
                        && !ed301v2_pki_verify_request(mutated, leaf_key),
            ...
            /* Public API mutations of the signature and embedded SPKI. */
            {
                X509_REQ *mutated = reparse_request_der(req);
                const ASN1_BIT_STRING *mutated_signature = NULL;
                unsigned char shortened[ED301V2_SIG_BYTES - 1];
                int set_ok = 0;
                ...
                if (mutated != NULL) {
                    ...
                    if (mutated_signature != NULL
                            && ed301v2_bit_string_length(mutated_signature)
                                == ED301V2_SIG_BYTES) {
                        memcpy(shortened,
                            ASN1_STRING_get0_data(mutated_signature),
                            sizeof(shortened));
                        set_ok = ed301v2_bit_string_set(
                            (ASN1_BIT_STRING *)mutated_signature,
                            shortened, sizeof(shortened));
                    }
                }
                ED301V2_CHECK(mutated != NULL && set_ok == 1
                        && !ed301v2_pki_verify_request(mutated, leaf_key),
...

            {
                ...
                ED301V2_CHECK(mutated != NULL && set_ok
                        && !ed301v2_pki_verify_request(mutated, leaf_key),
...

            {
                ...
                if (public_key != NULL
                        && X509_PUBKEY_get0_param(&object, &key_bytes,
                            &key_length, &algorithm, public_key) == 1
                        && key_bytes != NULL
                        && key_length == (int)ED301V2_PUB_BYTES) {
                    short_key = OPENSSL_memdup(
                        key_bytes, ED301V2_PUB_BYTES - 1);
                    target = OBJ_txt2obj(ED301V2_OID_TEXT, 1);
                    if (short_key != NULL && target != NULL
                            && X509_PUBKEY_set0_param(public_key, target,
                                V_ASN1_UNDEF, NULL, short_key,
                                (int)ED301V2_PUB_BYTES - 1) == 1) {
                        target = NULL;
                        short_key = NULL;
                        set_ok = 1;
                    }
     ...
     */
    {
        X509 *ca_cert = make_cert("Ed301-EdDSA-v2 test CA", NULL, ca_key,
            ca_key, 1, 1);
        X509 *leaf_cert = NULL;

        ED301V2_CHECK(ca_cert != NULL, "self-signed CA certificate");
        ED301V2_CHECK(ca_cert != NULL
                && ed301v2_pki_verify_certificate(ca_cert, ca_key),
            "strict certificate wrapper verifies the self-signed CA");
        if (ca_cert != NULL) {
            X509 *der_copy = reparse_certificate_der(ca_cert);

            ED301V2_CHECK(der_copy != NULL
                    && ed301v2_pki_verify_certificate(der_copy, ca_key),
                "P1 self-signed Ed301 certificate DER round trip verifies");
            X509_free(der_copy);
        }

        if (ca_cert != NULL) {
            leaf_cert = make_cert("Ed301-EdDSA-v2 test leaf",
                X509_get_subject_name(ca_cert), leaf_key, ca_key, 0, 2);
            ED301V2_CHECK(leaf_cert != NULL, "CA-signed leaf certificate");
            ED301V2_CHECK(ed301v2_pki_certificate_is_exact(leaf_cert),
            ...
            if (leaf_cert != NULL) {
                ...
                ED301V2_CHECK(der_copy != NULL
                    ...
                    "exact profile");
                ED301V2_CHECK(pem_copy != NULL
                        && ed301v2_pki_verify_certificate(pem_copy, ca_key),
        ...
        if (ca_cert != NULL && leaf_cert != NULL) {
            X509_STORE *store = X509_STORE_new();
            X509_STORE_CTX *store_ctx = X509_STORE_CTX_new_ex(pki_libctx, NULL);

            ED301V2_CHECK(store != NULL && store_ctx != NULL
                    && X509_STORE_add_cert(store, ca_cert) == 1
                    && X509_STORE_CTX_init(store_ctx, store, leaf_cert,
                        NULL) == 1
                    && ed301v2_pki_verify_two_certificate_chain(
            ...
            /* Corrupt the leaf signature: chain must fail. */
            {
                const ASN1_BIT_STRING *signature = NULL;
                const X509_ALGOR *algorithm = NULL;
                X509_STORE_CTX *bad_ctx = X509_STORE_CTX_new_ex(pki_libctx, NULL);

                X509_get0_signature(&signature, &algorithm, leaf_cert);
                if (signature != NULL
                        && ed301v2_bit_string_length(signature) > 10) {
                    unsigned char *bytes = (unsigned char *)
                        ASN1_STRING_get0_data(signature);

                    bytes[10] ^= 1;
                    ED301V2_CHECK(bad_ctx != NULL
            ...
            ED301V2_CHECK(!ed301v2_pki_verify_certificate(leaf_cert, leaf_key),
             ...
             */
            {
                X509 *mutated = reparse_certificate_der(leaf_cert);
                ...
                ED301V2_CHECK(set_ok
                        && !ed301v2_pki_verify_certificate(mutated, ca_key),
        ...
        if (ca_cert != NULL) {
            ...
            ED301V2_CHECK(ec_ca_cert != NULL && ecdsa_to_ed_leaf != NULL
                    && ed301v2_pki_public_key_is_exact(
                        X509_get_X509_PUBKEY(ecdsa_to_ed_leaf))
                    && !ed301v2_pki_certificate_is_exact(ecdsa_to_ed_leaf)
                    && verify_direct_chain(ec_ca_cert, ecdsa_to_ed_leaf),
                "P3 classic ECDSA CA-to-Ed301 leaf verifies generically");
            ED301V2_CHECK(ed_to_ecdsa_leaf != NULL
                    && !ed301v2_pki_public_key_is_exact(
                        X509_get_X509_PUBKEY(ed_to_ecdsa_leaf))
                    && !ed301v2_pki_certificate_is_exact(ed_to_ecdsa_leaf)
    ...
    /* Root -> intermediate -> leaf, plus the RFC 5280 CRL boundary. */
    {
        ...
        X509 *bad_intermediate = intermediate == NULL ? NULL
            : reparse_certificate_der(intermediate);
        X509_CRL *bad_crl = NULL;

        /* Keep the negative control bound to the private context. A
         * generic duplicate may otherwise lose its provider association. */
        if (crl != NULL) {
            unsigned char *der = NULL;
            int length = i2d_X509_CRL(crl, &der);
            const unsigned char *cursor = der;

            bad_crl = X509_CRL_new_ex(pki_libctx, NULL);
            if (length <= 0 || bad_crl == NULL
                    || d2i_X509_CRL(&bad_crl, &cursor, length) == NULL
                    || cursor != der + length) {
        ...
        ED301V2_CHECK(root != NULL && intermediate != NULL
                && revoked_leaf != NULL && valid_leaf != NULL
                && ed301v2_pki_certificate_is_exact(root)
                && ed301v2_pki_certificate_is_exact(intermediate)
                && ed301v2_pki_certificate_is_exact(revoked_leaf)
                && verify_intermediate_chain(
                    root, intermediate, revoked_leaf, NULL, 0),
            "three-level Ed301 root/intermediate/leaf chain verifies");
        ED301V2_CHECK(root != NULL && revoked_leaf != NULL
        ...
        ED301V2_CHECK(root != NULL && bad_intermediate != NULL
                && revoked_leaf != NULL
                && !verify_intermediate_chain(
                    root, bad_intermediate, revoked_leaf, NULL, 0),
            "three-level chain rejects a corrupted intermediate");

        ED301V2_CHECK(crl != NULL && intermediate != NULL
                && ed301v2_pki_crl_is_exact(crl)
        ...
        ED301V2_CHECK(bad_crl != NULL
                && ed301v2_pki_crl_is_exact(bad_crl)
                && X509_CRL_verify(bad_crl, intermediate_key) == 1,
            "private-context reparsed CRL verifies before signature corruption");
        if (bad_crl != NULL) {
            const ASN1_BIT_STRING *signature = NULL;
            const X509_ALGOR *algorithm = NULL;
...
cleanup:
    ED301V2_CHECK(OSSL_PROVIDER_available(NULL, ED301V2_PKI_PROVIDER) == 0,
        "v2 PKI provider was never loaded into the default context");
    EVP_PKEY_free(ca_key);
    EVP_PKEY_free(intermediate_key);
    EVP_PKEY_free(leaf_key);
    OSSL_PROVIDER_unload(v1);
    OSSL_PROVIDER_unload(deflt);
    OSSL_LIB_CTX_free(pki_libctx);
    pki_libctx = NULL;
    return ed301v2_summary("provider_pki");
}
