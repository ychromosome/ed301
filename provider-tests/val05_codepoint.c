 ...
 * CertificateVerify with 0xfe85 (one outgoing and one incoming observation),
...
typedef struct tls_outcome_st {
    ...
    TRACE trace;
    CAPABILITY_CAPTURE v2_capability;
    ...
    OSSL_PROVIDER *provider, CAPABILITY_CAPTURE *capture)
{
    ...
    return capture->entries == 1
        && capture->code_point == ED301V2_TLS_SIGALG_CODE_POINT;
}
...
static X509 *make_self_signed(OSSL_LIB_CTX *libctx,
    EVP_PKEY *pkey, const char *common_name)
{
    X509 *certificate = X509_new_ex(libctx, NULL);
...
static int exact_certificate_verify(const TRACE *trace)
{
    ...
    return first->endpoint == 'S' && first->outgoing
        && first->scheme == ED301V2_TLS_SIGALG_CODE_POINT
        && first->signature_length == ED301V2_SIG_BYTES
        && first->wire_length == ED301V2_CV_WIRE_BYTES
        && second->endpoint == 'C' && !second->outgoing
        && second->scheme == ED301V2_TLS_SIGALG_CODE_POINT
        && second->signature_length == ED301V2_SIG_BYTES
        && second->wire_length == ED301V2_CV_WIRE_BYTES;
}
...
static void print_outcome(const char *order, const TLS_OUTCOME *outcome)
{
    ...
    printf("VAL05_CAPABILITY v2=%d collider=%d v2_iana=%s "
        "v2_name=%s v2_keytype=%s collider_iana=%s "
        "collider_name=%s collider_keytype=%s v2_cp=0x%04x "
        "collider_cp=0x%04x collider_keymgmt_owned=%d\n",
        capability_matches(&outcome->v2_capability,
            ED301V2_TLS_SIGALG_IANA_NAME, ED301V2_ALG, ED301V2_ALG),
        capability_matches(&outcome->collider_capability,
            ED301V2_TLS_SIGALG_IANA_NAME, ED301V2_ALG, ED301V2_ALG),
        outcome->v2_capability.iana_name,
        outcome->v2_capability.sigalg_name,
        outcome->v2_capability.keytype,
        outcome->collider_capability.iana_name,
        outcome->collider_capability.sigalg_name,
        outcome->collider_capability.keytype,
        outcome->v2_capability.code_point,
...
{
    const int v2_first = strcmp(order, "v2-then-collider") == 0;
    OSSL_LIB_CTX *libctx = NULL;
    OSSL_PROVIDER *default_provider = NULL;
    OSSL_PROVIDER *v2_provider = NULL;
    ...
    if (!v2_first && strcmp(order, "collider-then-v2") != 0) {
        snprintf(outcome.setup_stage, sizeof(outcome.setup_stage),
            "bad-order");
        print_outcome(order, &outcome);
        ED301V2_CHECK(0, "unknown child order '%s'", order);
        return ed301v2_summary("val05_codepoint-child");
    }

    ERR_clear_error();
    libctx = OSSL_LIB_CTX_new();
    if (libctx == NULL) {
        snprintf(outcome.setup_stage, sizeof(outcome.setup_stage), "private-libctx");
        goto done;
    }
    default_provider = OSSL_PROVIDER_load(libctx, "default");
    if (v2_first) {
        v2_provider = ed301v2_load_named(libctx, NULL, ED301V2_TLS_PROVIDER);
        collider_provider =
            ed301v2_load_named(libctx, NULL, ED301V2_TLS_COLLIDER_PROVIDER);
    } else {
        collider_provider =
            ed301v2_load_named(libctx, NULL, ED301V2_TLS_COLLIDER_PROVIDER);
        v2_provider = ed301v2_load_named(libctx, NULL, ED301V2_TLS_PROVIDER);
    ...
    outcome.provider_load_ok = default_provider != NULL
        && v2_provider != NULL && collider_provider != NULL;
    ...
    outcome.capability_ok = inspect_capability(
            v2_provider, &outcome.v2_capability)
    ...
    collider_keymgmt = EVP_KEYMGMT_fetch(
        libctx, ED301V2_ALG, ED301V2_TLS_COLLIDER_PROP);
    ...
    if (keymgmt_owner != NULL) {
        ...
        outcome.collider_keymgmt_owned = owner_name != NULL
            && strcmp(owner_name, ED301V2_TLS_COLLIDER_PROVIDER) == 0;
    }
    EVP_KEYMGMT_free(collider_keymgmt);
    collider_keymgmt = NULL;
    ERR_clear_error();

    server_key = ed301v2_keygen(libctx);
    if (server_key == NULL) {
        snprintf(outcome.setup_stage, sizeof(outcome.setup_stage),
            "v2-keygen");
        record_error_queue(outcome.error_text, sizeof(outcome.error_text));
    ...
    server_certificate = make_self_signed(libctx, server_key,
    ...
    server_context = SSL_CTX_new_ex(libctx, NULL, TLS_server_method());
    client_context = SSL_CTX_new_ex(libctx, NULL, TLS_client_method());
    ...
    if (v2_provider != NULL)
        OSSL_PROVIDER_unload(v2_provider);
    if (default_provider != NULL)
        OSSL_PROVIDER_unload(default_provider);
    OSSL_LIB_CTX_free(libctx);
    ED301V2_CHECK(OSSL_PROVIDER_available(NULL, ED301V2_TLS_PROVIDER) == 0
            && OSSL_PROVIDER_available(NULL, ED301V2_TLS_COLLIDER_PROVIDER) == 0,
        "v2 collision fixtures absent from the default context");
    return ed301v2_summary("val05_codepoint-child");
}
...
int main(int argc, char **argv)
{
    ...
    v2_spawned = spawn_child(self, "v2-then-collider", &v2_first);
    collider_spawned = spawn_child(self, "collider-then-v2", &collider_first);
    print_child_capture("v2-then-collider", &v2_first);
    print_child_capture("collider-then-v2", &collider_first);
    ED301V2_CHECK(v2_spawned && child_result_accepted(&v2_first)
            && v2_first.exit_code == 0 && !v2_first.timed_out
            && !v2_first.signaled && !v2_first.output_truncated,
        "v2-then-collider fresh child exits zero with accepted result");
    ED301V2_CHECK(collider_spawned && child_result_accepted(&collider_first)
            && collider_first.exit_code == 0 && !collider_first.timed_out
            && !collider_first.signaled && !collider_first.output_truncated,
        "collider-then-v2 fresh child exits zero with accepted result");
    ED301V2_CHECK(child_result_accepted(&v2_first)
            && child_result_accepted(&collider_first)
            && strcmp(v2_first.result_line,
                collider_first.result_line) == 0,
        "provider load orders have identical outcome/error/alert result");
    return ed301v2_summary("val05_codepoint");
}
