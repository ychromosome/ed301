...
static void *concurrent_keygen_worker(void *argument)
{
    ...
    key = ed301v2_keygen(worker->libctx);
    worker->ok = key != NULL
...
static int concurrent_default_keygen_is_distinct(void)
{
    ...
    unsigned char zero[ED301V2_PUB_BYTES] = { 0 };
    size_t created = 0;
    size_t index;
    size_t other;
    int mutex_ready = 0;
    ...
    if (libctx == NULL
            ...
            || (v2 = OSSL_PROVIDER_load(libctx, ED301V2_PROVIDER)) == NULL)
        goto done;
...
static void *lifecycle_keygen_worker(void *argument)
{
    ...
    EVP_PKEY *key = ed301v2_keygen(worker->libctx);
...
int main(void)
{
    ...
    unsigned char expected[ED301V2_SEED_BYTES];
    const unsigned char *expected_public = ED301V2_TEST_RAND_PUBLIC;
    unsigned char actual[ED301V2_SEED_BYTES];
    unsigned char actual_public[ED301V2_PUB_BYTES];
    unsigned char deterministic_signature[ED301V2_SIG_BYTES];
    ...
    ED301V2_CHECK(ed301_first_load_has_no_child_fallback(),
        "Ed301-first load remains keygen-disabled until application RAND "
        "becomes available");
    ED301V2_CHECK(concurrent_default_keygen_is_distinct(),
    ...
    ED301V2_CHECK(libctx != NULL, "parent library context");
    ED301V2_CHECK(libctx != NULL
    ...
    ED301V2_CHECK(rand_provider != NULL, "application test RAND provider loads");
    ED301V2_CHECK(libctx != NULL
     ...
     * expected public key is generated from the a0..c5 public test seed
     * by the hash-bound Phase-B Python reference in gen_vectors.py.
     * It is not read back from provider behaviour.
     */
    ED301V2_CHECK(key != NULL && test_rand_generate_calls > 0,
        "keygen reaches the application-selected OpenSSL RAND provider");
    ED301V2_CHECK(key != NULL && test_rand_generate_strength >= 149U,
    ...
    ED301V2_CHECK(EVP_set_default_properties(libctx, ED301V2_PROP) == 1,
        "application policy can tighten after DRBG creation");
    warm_policy_key = ed301v2_keygen(libctx);
    ED301V2_CHECK(warm_policy_key != NULL,
        "warm provider DRBG remains usable after a policy change");
    ED301V2_CHECK(EVP_set_default_properties(
     ...
     */
    kat_key = ed301v2_key_from_seed(libctx, POSITIVE_CASES[0].seed);
    calls_before_failure = test_rand_generate_calls;
    test_rand_fail = 1;
    ED301V2_CHECK(kat_key != NULL
            && ed301v2_digest_sign(libctx, kat_key,
            ...
            && ed301v2_digest_verify(libctx, kat_key,
                POSITIVE_CASES[0].message,
                POSITIVE_CASES[0].message_len,
                deterministic_signature,
    ...
    calls_before_failure = test_rand_generate_calls;
    weak_key = ed301v2_keygen(libctx);
    ED301V2_CHECK(weak_key == NULL
            && test_rand_generate_calls >= calls_before_failure,
    ...
    calls_before_failure = test_rand_generate_calls;
    test_rand_fail = 1;
    failed_key = ed301v2_keygen(libctx);
    ED301V2_CHECK(failed_key == NULL
            && test_rand_generate_calls > calls_before_failure,
        "application RAND failure makes Ed301 keygen fail closed");
    ERR_clear_error();

    test_rand_fail = 0;
    recovered_key = ed301v2_keygen(libctx);
    actual_length = 0;
    actual_public_length = 0;
    ED301V2_CHECK(recovered_key != NULL
            && EVP_PKEY_get_octet_string_param(
    ...
    ED301V2_CHECK(synchronization_ready,
    ...
    if (worker_started) {
        ...
        while (!worker.ready)
            pthread_cond_wait(&worker.condition, &worker.mutex);
        pthread_mutex_unlock(&worker.mutex);
        ED301V2_CHECK(worker.keygen_ok,
        ...
        EVP_PKEY_free(key);
        key = NULL;
        ED301V2_CHECK(OSSL_PROVIDER_unload(v2) == 1,
    ...
    return ed301v2_summary("provider_rand");
}
