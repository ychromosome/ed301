/*
 * Acceptance section 3 (signature contract): the Phase-B v2
 * signing, point, scalar and verification corpus,
...
static const unsigned char ED301V2_EXPECTED_ALGORITHM_ID[15] = {
    0x30, 0x0d, 0x06, 0x0b, 0x2b, 0x06, 0x01, 0x04,
    0x01, 0x84, 0x85, 0x6a, 0x82, 0x2d, 0x05
};

/*
 * FBL-08 mutation control: with ED301V2_POLICY_MUTATE=1 every expected
 ...
 * that failure.
 */
static int ed301v2_policy_invert(void)
{
    const char *value = getenv("ED301V2_POLICY_MUTATE");
 ...
 */
static int ed301v2_message_sign_init_rejects(OSSL_LIB_CTX *libctx, EVP_PKEY *pkey,
    const OSSL_PARAM *params)
{
    EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
...
int main(void)
{
    ...
    OSSL_PROVIDER *v1 = ed301v2_load(libctx, &deflt);
    size_t index;
     ...
     */
    for (index = 0; index < sizeof(POSITIVE_CASES) / sizeof(POSITIVE_CASES[0]); index++) {
        ...
        ED301V2_CHECK(pctx != NULL
                && ed301v2_sign_message_init(libctx, pctx, NULL),
        ...
        ED301V2_CHECK(pctx != NULL
                && EVP_PKEY_sign(pctx, sig, &sig_len, tc->message,
                    tc->message_len) == 1
                && sig_len == ED301V2_SIG_BYTES
                && memcmp(sig, tc->signature, ED301V2_SIG_BYTES) == 0,
        ...
        ED301V2_CHECK(pctx != NULL
                && EVP_PKEY_sign(pctx, sig_again, &sig_len, tc->message,
                    tc->message_len) == 1
                && memcmp(sig, sig_again, ED301V2_SIG_BYTES) == 0,
            "%s: deterministic", tc->id);
        EVP_PKEY_CTX_free(pctx);

        /* One-shot DigestSign agrees exactly. */
        memset(sig_again, 0, sizeof(sig_again));
        ED301V2_CHECK(ed301v2_digest_sign(libctx, pkey, tc->message,
                tc->message_len, sig_again)
                && memcmp(sig_again, tc->signature, ED301V2_SIG_BYTES) == 0,
         ...
         */
        {
            ...
            unsigned char small[ED301V2_SIG_BYTES - 1];
            unsigned char canary[ED301V2_SIG_BYTES - 1];
            size_t digest_sig_len = 0;

            ED301V2_CHECK(size_context != NULL
                    && EVP_DigestSignInit_ex(size_context, NULL, NULL,
                        libctx, ed301v2_property, pkey, NULL) == 1
                    && EVP_DigestSign(size_context, NULL, &digest_sig_len,
                        tc->message, tc->message_len) == 1
                    && digest_sig_len == ED301V2_SIG_BYTES,
                "%s: DigestSign size query returns exactly 76", tc->id);

            memset(small, 0x5a, sizeof(small));
            memset(canary, 0x5a, sizeof(canary));
            digest_sig_len = sizeof(small);
            ED301V2_CHECK(small_context != NULL
        ...
        pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        ED301V2_CHECK(pctx != NULL && EVP_PKEY_verify_init(pctx) != 1,
        ...
        pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        ED301V2_CHECK(pctx != NULL
                && ed301v2_verify_message_init(libctx, pctx, NULL)
                && EVP_PKEY_verify(pctx, tc->signature, ED301V2_SIG_BYTES,
                    tc->message, tc->message_len) == 1,
            "%s: whole-message EVP_PKEY_verify accepts", tc->id);
        EVP_PKEY_CTX_free(pctx);
        ED301V2_CHECK(ed301v2_digest_verify(libctx, pkey, tc->message,
                tc->message_len, tc->signature, ED301V2_SIG_BYTES),
            "%s: DigestVerify accepts", tc->id);

        EVP_PKEY_free(pkey);
    }

    /* Ed448 pattern translated to Ed301-v2: the one-octet length binds
    ...
    for (index = 0;
        ...
        OSSL_PARAM params[2];
        OSSL_PARAM query[2];
        unsigned char signature[ED301V2_SIG_BYTES] = { 0 };
        unsigned char duplicate_signature[ED301V2_SIG_BYTES] = { 0 };
        ...
        sign_ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        ED301V2_CHECK(sign_ctx != NULL
                && ed301v2_sign_message_init(libctx, sign_ctx, params)
        ...
        signature_length = sizeof(signature);
        ED301V2_CHECK(sign_ctx != NULL
                && ed301v2_sign_message_init(libctx, sign_ctx, NULL)
                && EVP_PKEY_CTX_get_params(sign_ctx, query) == 1
        ...
        verify_ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        ED301V2_CHECK(verify_ctx != NULL
                && ed301v2_verify_message_init(libctx, verify_ctx, params)
                && EVP_PKEY_verify(verify_ctx, tc->signature,
                    ED301V2_SIG_BYTES, tc->message, tc->message_len) == 1,
            "%s: matching native context verifies", tc->id);

        ED301V2_CHECK(tc->context_len != 0,
        ...
        ED301V2_CHECK(verify_ctx != NULL
                ...
                && EVP_PKEY_verify(verify_ctx, tc->signature,
                    ED301V2_SIG_BYTES, tc->message, tc->message_len) == 0,
        ...
        params[0] = OSSL_PARAM_construct_octet_string(
            ...
            (void *)tc->context, tc->context_len);
        signature_length = sizeof(signature);
        ED301V2_CHECK(digest_ctx != NULL
                && EVP_DigestSignInit_ex(digest_ctx, NULL, NULL, libctx,
                    ED301V2_PROP, pkey, params) == 1
    ...
    /* Context parameter failures are atomic and preserve the prior domain. */
    {
        ...
        EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_from_pkey(
            libctx, pkey, ED301V2_PROP);
        unsigned char oversized[256] = { 0 };
        unsigned char output[ED301V2_SIG_BYTES];
        unsigned char canary[ED301V2_SIG_BYTES];
        char wrong_type[] = "ctx";
        OSSL_PARAM params[3];
        ...
        ED301V2_CHECK(pctx != NULL
                ...
                && EVP_PKEY_sign(pctx, output, &output_length,
                    tc->message, tc->message_len) == 1
                && output_length == ED301V2_SIG_BYTES
    ...
    /*
     * S4 -- Ed301-v2 deterministic-signing contract: five independent one-shot
     * operations over the same key/message produce the exact KAT bytes.
     */
    {
        const POSITIVE_CASE *tc = &POSITIVE_CASES[0];
        EVP_PKEY *pkey = ed301v2_key_from_seed(libctx, tc->seed);
        unsigned char signatures[5][ED301V2_SIG_BYTES];
        int deterministic = pkey != NULL;
        size_t repetition;

        for (repetition = 0; deterministic && repetition < 5; repetition++)
            deterministic = ed301v2_digest_sign(libctx, pkey,
    ...
    /* NULL-key reinitialization follows the built-in EdDSA lifecycle. */
    {
        ...
        ED301V2_CHECK(pkey != NULL && sign_context != NULL
                && EVP_DigestSignInit_ex(sign_context, NULL, NULL, libctx,
                    ED301V2_PROP, pkey, NULL) == 1
        ...
        ED301V2_CHECK(sign_context != NULL
                && EVP_DigestSignInit_ex(sign_context, NULL, NULL, libctx,
                    ED301V2_PROP, NULL, NULL) == 1,
        ...
        ED301V2_CHECK(builtin_eddsa_null_key_reinit_control(
                libctx, "ED25519", 64),
     ...
     */
    {
        ...
        unsigned char malformed[ED301V2_SIG_BYTES];
        unsigned char overlong[ED301V2_SIG_BYTES + 1];
        int result;

        ERR_clear_error();
        result = pkey == NULL ? -1
            : ed301v2_digest_verify_result(libctx, pkey,
                changed_message, sizeof(changed_message),
                tc->signature, ED301V2_SIG_BYTES);
        ED301V2_CHECK(result == 0,
        ...
        /* S3 -- Ed301-v2 signature encoding is exactly 76 bytes. */
        ERR_clear_error();
        result = pkey == NULL ? -1
            : ed301v2_digest_verify_result(libctx, pkey,
                tc->message, tc->message_len,
                tc->signature, ED301V2_SIG_BYTES - 1);
        ED301V2_CHECK(result == 0,
            "75-byte signature returns exactly zero");

        memcpy(overlong, tc->signature, ED301V2_SIG_BYTES);
        overlong[ED301V2_SIG_BYTES] = 0;
        ERR_clear_error();
        result = pkey == NULL ? -1
            : ed301v2_digest_verify_result(libctx, pkey,
                tc->message, tc->message_len,
        ...
        result = pkey == NULL ? 0
            : ed301v2_digest_verify_result(libctx, pkey,
                tc->message, tc->message_len,
                NULL, ED301V2_SIG_BYTES);
        ED301V2_CHECK(result < 0 && ERR_peek_error() != 0,
            "NULL signature buffer returns negative with provider error");
        ERR_clear_error();

        result = pkey == NULL ? 0
            : ed301v2_digest_verify_result(libctx, pkey,
                NULL, 1, tc->signature, ED301V2_SIG_BYTES);
        ED301V2_CHECK(result < 0 && ERR_peek_error() != 0,
     ...
     * forms a slice, using a one-byte pointer as the bounded storage. */
    {
        ...
        ED301V2_CHECK(pkey != NULL, "oversized-message: key");
        memset(output, 0xa5, sizeof(output));
        memcpy(canary, output, sizeof(canary));

        pctx = pkey == NULL ? NULL
            : EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        if (pctx != NULL && ed301v2_sign_message_init(libctx, pctx, NULL))
            sign_failed = EVP_PKEY_sign(pctx, output, &sig_len,
                &one_byte, oversized) != 1
                && memcmp(output, canary, sizeof(output)) == 0;
        ED301V2_CHECK(sign_failed,
            "oversized-message: EVP_PKEY_sign fails closed above isize::MAX");
        EVP_PKEY_CTX_free(pctx);

        pctx = pkey == NULL ? NULL
            : EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        if (pctx != NULL && ed301v2_verify_message_init(libctx, pctx, NULL))
            verify_failed = EVP_PKEY_verify(pctx, tc->signature,
                ED301V2_SIG_BYTES, &one_byte, oversized) < 0;
        ED301V2_CHECK(verify_failed,
            "oversized-message: EVP_PKEY_verify returns an operational error");
        EVP_PKEY_CTX_free(pctx);

        memset(output, 0, sizeof(output));
        ED301V2_CHECK(pkey != NULL
                && ed301v2_digest_sign(libctx, pkey, tc->message,
                    tc->message_len, output)
                && memcmp(output, tc->signature, ED301V2_SIG_BYTES) == 0
                && ed301v2_digest_verify(libctx, pkey, tc->message,
                    tc->message_len, output, ED301V2_SIG_BYTES),
            "oversized-message: normal sign and verify recover");
        EVP_PKEY_free(pkey);
    }

    /* Complete point matrix: provider public-key policy surface. */
    for (index = 0; index < sizeof(POINT_CASES) / sizeof(POINT_CASES[0]); index++) {
        const POINT_CASE *tc = &POINT_CASES[index];
        EVP_PKEY *pkey = ed301v2_key_from_public(
         ...
         */
        {
            ...
            size_t forged_len = tc->encoding_len + ED301V2_SEED_BYTES;

            memcpy(forged, tc->encoding, tc->encoding_len);
            memcpy(forged + tc->encoding_len,
                POSITIVE_CASES[0].signature + 38, 38);
            ED301V2_CHECK(!ed301v2_triple_accepts(libctx,
                    POSITIVE_CASES[0].public_key, ED301V2_PUB_BYTES,
    ...
    /* Complete scalar matrix: provider S-parse surface. */
    for (index = 0; index < sizeof(SCALAR_CASES) / sizeof(SCALAR_CASES[0]); index++) {
    ...
    /* Every Phase-B verification row, including native-context failures. */
    for (index = 0; index < sizeof(VERIFICATION_CASES) / sizeof(VERIFICATION_CASES[0]); index++) {
        const VERIFICATION_CASE *tc = &VERIFICATION_CASES[index];
        EVP_PKEY *pkey = ed301v2_key_from_public(
            libctx, tc->public_key, tc->public_key_len);
        ...
        accepted = pkey != NULL && context != NULL
            && EVP_DigestVerifyInit_ex(context, NULL, NULL, libctx,
                ED301V2_PROP, pkey, parameters) == 1
            && EVP_DigestVerify(context, tc->signature, tc->signature_len,
                tc->message, tc->message_len) == 1;
        ED301V2_CHECK(accepted == (tc->expect_accept ^ invert),
    ...
    for (index = 0;
        ...
        int accepted = ed301v2_triple_accepts(libctx,
            tc->public_key, ED301V2_PUB_BYTES,
            tc->message, tc->message_len,
    ...
    /* 77 deterministic negative mutations from the reference lane. */
    {
        ...
        for (index = 0; index < 77; index++) {
            ...
            if (!accepted)
                rejected_count++;
            ED301V2_CHECK(!accepted, "negative %zu (%s) must be rejected",
                index, tc->label);
        }
        ED301V2_CHECK(rejected_count == 77,
    ...
    {
        EVP_PKEY *v1_key = ed301v2_key_from_seed(
            libctx, POSITIVE_CASES[0].seed);

        ED301V2_CHECK(memcmp(HISTORICAL_PUBLIC_KEY,
                POSITIVE_CASES[0].public_key, ED301V2_PUB_BYTES) != 0,
            "same seed yields different Ed301-EdDSA-v2 and Ed301-Sig-v1 "
            "public keys");
        ED301V2_CHECK(!ed301v2_triple_accepts(libctx,
                HISTORICAL_PUBLIC_KEY, ED301V2_PUB_BYTES,
            NULL, 0,
            HISTORICAL_SIGNATURE, sizeof(HISTORICAL_SIGNATURE)),
            "historical signature under historical public key does not "
            "verify as Ed301-EdDSA-v2");
        ED301V2_CHECK(v1_key != NULL
                && !ed301v2_digest_verify(libctx, v1_key, NULL, 0,
                    HISTORICAL_SIGNATURE, sizeof(HISTORICAL_SIGNATURE)),
    ...
    /* Mode rejections. */
    {
        EVP_PKEY *pkey = ed301v2_key_from_seed(libctx, POSITIVE_CASES[0].seed);
        EVP_MD_CTX *mctx;
        EVP_PKEY_CTX *pctx = NULL;
        ...
        /*
         * S5 -- Ed301-v2 pure-EdDSA contract and OpenSSL
         ...
         */
        {
            ...
            for (digest_index = 0;
                ...
                ED301V2_CHECK(digest != NULL,
                    "%s is available for the external-digest rejection gate",
                    digest_name);

                mctx = EVP_MD_CTX_new();
                ED301V2_CHECK(pkey != NULL && digest != NULL && mctx != NULL
         ...
         */
        mctx = EVP_MD_CTX_new();
        ED301V2_CHECK(pkey != NULL && mctx != NULL
         ...
         * entry point.  Empty context is exactly the default transcript. */
        {
            ...
            pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            ED301V2_CHECK(pctx != NULL
                    && ed301v2_verify_message_init(libctx, pctx, params),
            ...
            /* Explicit empty context is the default Ed301-EdDSA-v2 domain. */
            pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            ED301V2_CHECK(pctx != NULL
                    && ed301v2_sign_message_init(libctx, pctx, NULL)
            ...
            pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            ED301V2_CHECK(pctx != NULL
                    && ed301v2_sign_message_init(libctx, pctx, empty_params),
            ...
            pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            ED301V2_CHECK(pctx != NULL
                    && ed301v2_verify_message_init(libctx, pctx, empty_params),
            ...
            ED301V2_CHECK(pkey != NULL && mctx != NULL
                    && EVP_DigestSignInit_ex(mctx, NULL, NULL, libctx,
                        ED301V2_PROP, pkey, empty_params) == 1,
         ...
         * binding still clears the previous Rust operation first. */
        {
            ...
            state_ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            memset(stale_sig, 0xa5, sizeof(stale_sig));
            memcpy(canary, stale_sig, sizeof(canary));
            stale_sig_len = sizeof(stale_sig);
            ED301V2_CHECK(ed301v2_digest_sign(libctx, pkey, probe,
                    sizeof(probe) - 1, expected_sig)
                    && state_ctx != NULL
                    && ed301v2_sign_message_init(libctx, state_ctx, NULL)
            ...
            memcpy(canary, stale_sig, sizeof(canary));
            stale_sig_len = sizeof(stale_sig);
            ED301V2_CHECK(state_ctx != NULL
                    && ed301v2_sign_message_init(libctx, state_ctx, NULL)
                    && !ed301v2_sign_message_init(
            ...
            state_ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
            ED301V2_CHECK(state_ctx != NULL
                    && ed301v2_verify_message_init(libctx, state_ctx, NULL)
                    && EVP_PKEY_CTX_set_params(state_ctx, bad_params) != 1
                    && EVP_PKEY_verify(state_ctx,
                        POSITIVE_CASES[0].signature, ED301V2_SIG_BYTES,
                        POSITIVE_CASES[0].message,
...

            {
                ...
                ED301V2_CHECK(digest_ctx != NULL
                        ...
                        && EVP_DigestSignInit_ex(digest_ctx, NULL,
                            "SHA256", libctx, ED301V2_PROP, pkey, NULL) != 1
        ...
        /* A successfully loaded provider publishes stable reason strings. */
        {
            ...
            ED301V2_CHECK(rejected && ERR_GET_REASON(error) == 7
                    && reason != NULL
        ...
        /* The advertised algorithm-id parameter is exact parameterless DER. */
        {
            unsigned char algorithm_id[
                sizeof(ED301V2_EXPECTED_ALGORITHM_ID)] = { 0 };
        ...
        /*
         * S7 -- Ed301-v2/provider pure-only contract.  OpenSSL 4.0
         * providers/implementations/signature/eddsa_sig.c
         * eddsa_set_ctx_params_internal() names the ph/ctx instance classes;
         * neither class is an Ed301-EdDSA-v2 mode.
         */
        {
        ...
        {
            EVP_PKEY *public_only = ed301v2_key_from_public(
                libctx, POSITIVE_CASES[0].public_key, ED301V2_PUB_BYTES);
     ...
     */
    {
        ...
        pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
        {
            ...
            for (ed301v2_i = 0; ed301v2_i < sizeof(sig); ed301v2_i++)
                sig[ed301v2_i] = (unsigned char)
                    (tc->signature[ed301v2_i % ED301V2_SIG_BYTES] ^ 0xff);
        }
        sig_len = sizeof(sig);
        ED301V2_CHECK(pctx != NULL
                && ed301v2_sign_message_init(libctx, pctx, NULL)
    ...
    return ed301v2_summary("provider_signature");
}
