...
static atomic_ulong allocation_countdown;
static atomic_ulong allocation_total;
...
static int hybrid_aliases_reject_atomically(
    OSSL_LIB_CTX *libctx, EVP_PKEY *private_key,
...
{
    unsigned char bad_public[HYBRID_PUBLIC_BYTES];
    unsigned char bad_ciphertext[HYBRID_CIPHERTEXT_BYTES];
    unsigned char recovered[HYBRID_SECRET_BYTES];
    unsigned int mask;
    int result;

    for (mask = 0x20; mask <= 0xe0; mask += 0x20) {
        memcpy(bad_public, canonical_public, sizeof(bad_public));
        memcpy(bad_ciphertext, canonical_ciphertext, sizeof(bad_ciphertext));
        bad_public[HYBRID_PUBLIC_BYTES - 1] |= (unsigned char)mask;
        bad_ciphertext[HYBRID_CIPHERTEXT_BYTES - 1] |= (unsigned char)mask;
        if (!rejected_public_mutation_leaves_key_empty(
                libctx, bad_public, sizeof(bad_public), canonical_public)
                || !decapsulation_failure_is_atomic(libctx, private_key,
                    bad_ciphertext, sizeof(bad_ciphertext), HYBRID_SECRET_BYTES))
            return 0;
    }
    for (mask = 0; mask < 3; mask++) {
        memcpy(bad_public, canonical_public, sizeof(bad_public));
        memcpy(bad_ciphertext, canonical_ciphertext, sizeof(bad_ciphertext));
        memcpy(bad_public + MLKEM_PUBLIC_BYTES, FIELD_MODULUS, X301_BYTES);
        memcpy(bad_ciphertext + MLKEM_CIPHERTEXT_BYTES, FIELD_MODULUS, X301_BYTES);
        bad_public[MLKEM_PUBLIC_BYTES] += (unsigned char)mask;
        bad_ciphertext[MLKEM_CIPHERTEXT_BYTES] += (unsigned char)mask;
        if (!rejected_public_mutation_leaves_key_empty(
                libctx, bad_public, sizeof(bad_public), canonical_public)
                || !decapsulation_failure_is_atomic(libctx, private_key,
                    bad_ciphertext, sizeof(bad_ciphertext), HYBRID_SECRET_BYTES))
    ...
    result = decapsulate(libctx, private_key, canonical_ciphertext,
            HYBRID_CIPHERTEXT_BYTES, recovered)
        && CRYPTO_memcmp(recovered, canonical_secret, sizeof(recovered)) == 0;
    ...
    const unsigned char valid[HYBRID_PUBLIC_BYTES])
{
    ...
    if (accepted > 0) {
        ...
        if (!x301_encoding_is_canonical(candidate + MLKEM_PUBLIC_BYTES))
            goto done;
...
typedef struct { int hybrid; int raw; } GROUP_COUNTS;

static int count_group_capability(const OSSL_PARAM params[], void *argument)
{
    GROUP_COUNTS *count = argument;
    const char *name = NULL;
    unsigned int id = 0, bits = 0, kem = 0;
    int minimum = 0, maximum = 0;

    if (count == NULL
            || !OSSL_PARAM_get_utf8_string_ptr(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_NAME), &name)
            || name == NULL
            || !OSSL_PARAM_get_uint(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_ID), &id)
            || !OSSL_PARAM_get_uint(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_SECURITY_BITS), &bits)
            || !OSSL_PARAM_get_uint(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_IS_KEM), &kem)
            || !OSSL_PARAM_get_int(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_MIN_TLS), &minimum)
            || !OSSL_PARAM_get_int(
                OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_MAX_TLS), &maximum)
            || minimum != 0x0304 || maximum != 0x0304)
        return 0;
    if (strcmp(name, HYBRID_NAME) == 0 && id == 0xfe2f && bits == 256 && kem == 1)
        count->hybrid++;
    else if (strcmp(name, X301_NAME) == 0 && id == 0xfe30 && bits == 149 && kem == 0)
        count->raw++;
    else
        return 0;
    return 1;
...
static int hybrid_fails_cleanly_without_mlkem(const char *module_directory)
{
    ...
    GROUP_COUNTS capability_count = { 0, 0 };
    int result = 0;
    ...
    result = (capability_count.hybrid == 0 && capability_count.raw == 1) && generated == NULL
        && OSSL_PROVIDER_available(libctx, DEFAULT_PROVIDER) == 0;
...
static int hybrid_x301_first_load_is_raw_only(const char *module_directory)
{
    ...
    result = raw_keymgmt != NULL
        && OSSL_PROVIDER_get_capabilities(x301, "TLS-GROUP",
               count_group_capability, &capability_count) == 1
        && (capability_count.hybrid == 0 && capability_count.raw == 1)
        && OSSL_PROVIDER_available(libctx, DEFAULT_PROVIDER) == 0;
    if (result) {
        deflt = OSSL_PROVIDER_load(libctx, DEFAULT_PROVIDER);
        capability_count = (GROUP_COUNTS){ 0, 0 };
        result = deflt != NULL
            && OSSL_PROVIDER_get_capabilities(x301, "TLS-GROUP",
                   count_group_capability, &capability_count) == 1
            && (capability_count.hybrid == 1 && capability_count.raw == 1);
    }
...
int main(int argc, char **argv)
{
    ...
    if (!hybrid_aliases_reject_atomically(
            libctx, private_key, public_bytes, ciphertext,
            original_secret)) {
        fail("v2 hybrid client and server aliases reject atomically");
        goto done;
    }
    pass("v2 hybrid client and server aliases reject atomically");
