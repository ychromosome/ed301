...
static int prepare_derive_slot(SLOT *slot, int class, int random_secret)
{
    ...
    if (class == 0)
        memcpy(material, random_secret ? SECRET_A : PUBLIC_B,
            sizeof(material));
    else if (RAND_bytes_ex(libctx, material, sizeof(material), 0) != 1)
        return 0;
    if (random_secret) {
        slot->key = raw_private(material);
        slot->peer = raw_public(PUBLIC_B);
    } else {
        slot->key = raw_private(SECRET_A);
        /* Uniform canonical u, including curve and twist: reject invalid
         * encodings outside the measured operation, never mask or reduce. */
        for (size_t tries = 0; tries < 128; tries++) {
            slot->peer = raw_public(material);
            if (slot->peer != NULL || class == 0)
                break;
            ERR_clear_error();
            if (RAND_bytes_ex(libctx, material, sizeof(material), 0) != 1)
                break;
        }
    }
...
static int prepare_slot(SLOT *slot, int class)
{
    ...
    case TEST_POSITIVE_CONTROL:
        ...
        else if (RAND_bytes_ex(libctx, slot->bytes, X301_BYTES, 0) != 1)
            return 0;
        return 1;
    ...
    case TEST_IMPORT_PRIVATE:
        if (class == 0)
            memcpy(slot->bytes, SECRET_A, X301_BYTES);
        else if (RAND_bytes_ex(libctx, slot->bytes, X301_BYTES, 0) != 1)
            return 0;
        return 1;
    ...
    int test, size_t total_measurements, double *max_t)
{
    ...
    *max_t = 0;
    for (i = 0; i < batches; i++) {
        dudect_state_t current = dudect_main(&ctx);

        if (current == DUDECT_LEAKAGE_FOUND)
            state = DUDECT_LEAKAGE_FOUND;
        for (size_t j = 0; j < DUDECT_TESTS; j++) {
            ttest_ctx_t *bucket = ctx.ttest_ctxs[j];
            if (bucket->n[0] > DUDECT_ENOUGH_MEASUREMENTS
                    && bucket->n[1] > DUDECT_ENOUGH_MEASUREMENTS) {
                double value = fabs(t_compute(bucket));
                if (!isfinite(value))
                    prepare_failures++;
                if (value > *max_t)
                    *max_t = value;
    ...
    if (ctx.ttest_ctxs[0]->n[0] <= DUDECT_ENOUGH_MEASUREMENTS
            || ctx.ttest_ctxs[0]->n[1] <= DUDECT_ENOUGH_MEASUREMENTS)
        prepare_failures++;
    dudect_free(&ctx);
    for (i = 0; i < BATCH_MEASUREMENTS; i++)
        slot_clear(&slots[i]);
    printf("%-24s peak_abs_t=%.2f prepare_failures=%zu "
