...
#ifndef X301_PROVIDER
# define X301_PROVIDER "x301_v2"
#endif
#define X301_PROPERTIES "provider=" X301_PROVIDER
...
static int hex_decode(
    const char *hex, unsigned char *output, size_t capacity, size_t *output_length)
{
    return hex != NULL && output != NULL && output_length != NULL
        && OPENSSL_hexstr2buf_ex(output, capacity, output_length, hex, '\0') == 1;
}
...
static int aliases_are_rejected_without_changing_keys(
    OSSL_LIB_CTX *libctx, EVP_PKEY *private_a, EVP_PKEY *canonical_public_a,
    EVP_PKEY *canonical_public_b)
{
    EVP_PKEY *candidate = NULL;
    EVP_PKEY *duplicate = EVP_PKEY_dup(canonical_public_b);
    unsigned char alias[X301_BYTES];
    unsigned char exported[X301_BYTES];
    unsigned char shared[X301_BYTES];
    unsigned int mask;
    int result = 0;
    ...
    for (mask = 0x20U; mask <= 0xe0U; mask += 0x20U) {
        ...
        candidate = raw_public(libctx, alias, sizeof(alias));
        if (candidate != NULL
                || EVP_PKEY_set1_encoded_public_key(duplicate, alias, sizeof(alias)) > 0
                || !export_raw_public_exact(duplicate, exported)
                || CRYPTO_memcmp(exported, PUBLIC_B, sizeof(exported)) != 0)
            goto done;
        ERR_clear_error();
        memcpy(alias, PUBLIC_A, sizeof(alias));
        alias[X301_BYTES - 1U] |= (unsigned char)mask;
        candidate = keypair_fromdata(libctx, SECRET_A, alias);
        if (candidate != NULL)
            goto done;
        ERR_clear_error();
    }
    for (mask = 0; mask < 4; mask++) {
        ...
        if (mask < 3)
            alias[0] += (unsigned char)mask; /* p's low byte leaves room for +2 */
        else {
            memset(alias, 0xff, sizeof(alias));
            alias[X301_BYTES - 1U] = 0x1f;
        }
        candidate = raw_public(libctx, alias, sizeof(alias));
        if (candidate != NULL)
            goto done;
    ...
    result = EVP_PKEY_eq(private_a, canonical_public_a) == 1
        && derive_exact(libctx, private_a, duplicate, shared)
        && CRYPTO_memcmp(shared, SHARED_AB, sizeof(shared)) == 0;
done:
    OPENSSL_cleanse(shared, sizeof(shared));
        ...
        if (strcmp(vector->operation, "public_from_secret") == 0) {
            if (strcmp(vector->expected, "valid") == 0) {
                ...
                case_ok = (strcmp(vector->expected_error, "secret_length") == 0
                        || strcmp(vector->expected_error, "weak_secret") == 0)
                    && private_key == NULL;
            }
        } else if (strcmp(vector->operation, "derive") == 0) {
            if (private_key == NULL) {
                case_ok = strcmp(vector->expected, "invalid") == 0
                    && (strcmp(vector->expected_error, "secret_length") == 0
                        || strcmp(vector->expected_error, "weak_secret") == 0);
                goto case_done;
            }
...
{
    unsetenv("X301_V2_PROVIDER_ALLOC_FAILPOINT");
    unsetenv("X301_V2_PROVIDER_PANIC_FAILPOINT");
}
    ...
    EVP_PKEY *peer)
{
    ...
    if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT", "key_new", 1) != 0)
        goto done;
    ...
    if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT", "key_import", 1) != 0)
        goto done;
    temporary_key = raw_private(libctx, SECRET_A, sizeof(SECRET_A));
    if (temporary_key != NULL
            || !derive_exact(libctx, private_key, peer, output)
            || memcmp(output, SHARED_AB, sizeof(output)) != 0)
        goto done;
    clear_x301_failpoints();
    ERR_clear_error();
    ...
    {
        static const char *const sites[] = { "key_generate_shared", "key_generate" };

        for (size_t i = 0; i < sizeof(sites) / sizeof(sites[0]); i++) {
            unsigned int before = rand_generate_calls;

            if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT", sites[i], 1) != 0)
                goto done;
            temporary_key = keygen(libctx);
            if (temporary_key != NULL || rand_generate_calls != before + 1)
                goto done;
            clear_x301_failpoints();
            ERR_clear_error();
            temporary_key = keygen(libctx);
    ...
    if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT", "key_duplicate", 1) != 0)
        goto done;
    ERR_clear_error();
    ...
    if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT", "exchange_new", 1) != 0)
        goto done;
    context = EVP_PKEY_CTX_new_from_pkey(
        libctx, private_key, X301_PROPERTIES);
    if (context != NULL && EVP_PKEY_derive_init(context) > 0)
    ...
    if (context == NULL || EVP_PKEY_derive_init(context) <= 0
            || EVP_PKEY_derive_set_peer(context, peer) <= 0
            || setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT",
    ...
    const char *site)
{
    ...
    if (strcmp(site, "key_new") == 0
        ...
        if (setenv("X301_V2_PROVIDER_PANIC_FAILPOINT", site, 1) != 0)
            goto done;
        temporary_key = raw_private(libctx, SECRET_A, sizeof(SECRET_A));
        ...
        context = EVP_PKEY_CTX_new_from_pkey(
            libctx, private_key, X301_PROPERTIES);
        if (context == NULL
                || setenv("X301_V2_PROVIDER_PANIC_FAILPOINT", site, 1) != 0)
            goto done;
...
int main(int argc, char **argv)
{
    ...
    unsigned char first[X301_BYTES];
    unsigned char second[X301_BYTES];
    const char *failpoint_mode = getenv("X301_V2_PROVIDER_FAILPOINT_MODE");
    ...
    if (failpoint_mode != NULL && (strcmp(failpoint_mode, "active") == 0
        ...
        if (strcmp(failpoint_mode, "active") == 0
                && !panic_failpoints_are_atomic(libctx, private_a, public_b)) {
        ...
        if (setenv("X301_V2_PROVIDER_ALLOC_FAILPOINT",
                "exchange_derive", 1) != 0
                || setenv("X301_V2_PROVIDER_PANIC_FAILPOINT",
                    "exchange_derive", 1) != 0
    ...
    if (!aliases_are_rejected_without_changing_keys(
            libctx, private_a, public_a, public_b)) {
        fail("v2 public aliases reject atomically; canonical keys remain usable");
        goto done;
    }
    pass("v2 public aliases reject atomically; canonical keys remain usable");
    ...
    rand_weak_remaining = 64;
    calls_before = rand_generate_calls;
    generated = keygen(libctx);
    if (generated == NULL || rand_weak_remaining != 0
            || rand_generate_calls != calls_before + 65U
            || !raw_roundtrip(generated, generated, TEST_RAND_SEED, TEST_RAND_PUBLIC)) {
        fail("v2 EVP keygen resamples all 64 weak aliases before one valid draw");
        goto done;
    }
    pass("v2 EVP keygen resamples all 64 weak aliases before one valid draw");
    EVP_PKEY_free(generated);
    generated = NULL;
    ...
    if (!adversarial_corpus_matches(libctx)) {
        fail("Phase-B, weak-secret, boundary and 512 DH oracle cases pass through EVP");
        goto done;
    }
    pass("Phase-B, weak-secret, boundary and 512 DH oracle cases pass through EVP");
