...
#define ED301V2_TLS_SIGALG_CODE_POINT 0xfe85
...
#define SERVER_NAME "server.v2.test.example"
#define CLIENT_NAME "client.v2.test.example"

static OSSL_LIB_CTX *tls_libctx;
static const char *tls_group_name = "X25519";
static int tls_group_nid = NID_X25519;
...
static EVP_PKEY *reload_private_key_through_pem(EVP_PKEY *source)
{
    OSSL_ENCODER_CTX *encoder = source == NULL ? NULL
        : OSSL_ENCODER_CTX_new_for_pkey(source, EVP_PKEY_KEYPAIR, "PEM",
            "PrivateKeyInfo", ED301V2_PKI_PROP);
    ...
    if (bio != NULL)
        loaded = PEM_read_bio_PrivateKey_ex(
            bio, NULL, NULL, NULL, tls_libctx, NULL);
    BIO_free(bio);
    ...
    size_t signature_length)
{
    ...
    valid = length == ED301V2_CV_WIRE_BYTES
        && length >= ED301V2_CV_WIRE_BYTES
        && bytes[1] == 0 && bytes[2] == 0 && bytes[3] == 80
        && bytes[6] == 0 && bytes[7] == (unsigned char)ED301V2_SIG_BYTES
    ...
    if (valid)
        memcpy(capture->signature, bytes + 8, ED301V2_SIG_BYTES);
    else
        trace->wire_overflow = 1;
}
    ...
    size_t expected_count)
{
    ...
    for (index = 0; index < expected_count; index++) {
        if (events[index].endpoint != expected[index].endpoint
                ...
                || events[index].signature_length != ED301V2_SIG_BYTES
                || events[index].length != ED301V2_CV_WIRE_BYTES)
            return 0;
    }
    return 1;
}
...
static X509_NAME *make_name(const char *common_name)
{
    ...
    if (name == NULL
            || X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC,
                (const unsigned char *)"Ed301-EdDSA-v2 TLS (test-only)",
                -1, -1, 0) != 1
...
static X509 *issue_cert(
    ...
    long serial)
{
    X509 *cert = X509_new_ex(tls_libctx, NULL);
    ...
    const EVP_MD *handshake_digest)
{
    ...
    return capture->tbs_length <= sizeof(capture->tbs)
        ...
        && hash_sha256(capture->tbs, capture->tbs_length,
            capture->tbs_sha256)
        && hash_sha256(capture->signature, ED301V2_SIG_BYTES,
...
static int provider_verify_cv(X509 *certificate, const CV_CAPTURE *capture)
{
    ...
    if (certificate_key != NULL
        ...
        ok = ed301v2_triple_accepts(tls_libctx, public_key, public_key_length,
    ...
    TLS_OUTCOME *outcome)
{
    ...
    for (index = 0; index < outcome->trace.capture_count; index++) {
        ...
        if (!capture->wire_valid
                || capture->scheme != ED301V2_TLS_SIGALG_CODE_POINT
...
static int run_tls(
    ...
    TLS_OUTCOME *outcome)
{
    SSL_CTX *server_ctx = SSL_CTX_new_ex(tls_libctx, NULL, TLS_server_method());
    SSL_CTX *client_ctx = SSL_CTX_new_ex(tls_libctx, NULL, TLS_client_method());
    SSL *server = NULL;
    SSL *client = NULL;
    ...
    stage = "groups";
    /* The TLS-1.2 case isolates the signature-version rejection; the
     * alternative v2 groups themselves are TLS-1.3-only. */
    if (SSL_CTX_set1_groups_list(server_ctx,
            options->force_tls12 ? "X25519" : tls_group_name) != 1
            || SSL_CTX_set1_groups_list(client_ctx,
                options->force_tls12 ? "X25519" : tls_group_name) != 1)
        goto done;
    ...
    if (outcome->handshake_ok) {
        ...
        outcome->group_expected =
            SSL_get_negotiated_group(client) == tls_group_nid
            && SSL_get_negotiated_group(server) == tls_group_nid;
        printf("TLS13_GROUP name=%s nid=0x%lx\n", tls_group_name,
            (unsigned long)SSL_get_negotiated_group(client));
...
static int configure_tls13_context(SSL_CTX *context)
{
    return context != NULL
        ...
        && SSL_CTX_set1_groups_list(context, tls_group_name) == 1;
}
...
static int sni_selects_ed301_certificate(
    ...
    EVP_PKEY *server_key)
{
    SSL_CTX *default_context = SSL_CTX_new_ex(tls_libctx, NULL, TLS_server_method());
    SSL_CTX *selected_context = SSL_CTX_new_ex(tls_libctx, NULL, TLS_server_method());
    SSL_CTX *client_context = SSL_CTX_new_ex(tls_libctx, NULL, TLS_client_method());
    SSL *server = NULL;
    SSL *client = NULL;
    X509 *peer = NULL;
    X509_STORE *store;
    ...
    if (!create_ssl_pair(
            default_context, client_context, &server, &client)
            || !set_client_server_name(client, "unknown.v2.test.example"))
        goto done;
...
int main(void)
{
    ...
    OSSL_PROVIDER *deflt = OSSL_PROVIDER_load(tls_libctx, "default");
    OSSL_PROVIDER *v1 =
        ed301v2_load_named(tls_libctx, NULL, ED301V2_TLS_PROVIDER);
    OSSL_PROVIDER *pki =
        ed301v2_load_named(tls_libctx, NULL, ED301V2_PKI_PROVIDER);
    OSSL_PROVIDER *x_provider = NULL;
    EVP_PKEY *ca_key = ed301v2_keygen(tls_libctx);
    EVP_PKEY *ca2_key = ed301v2_keygen(tls_libctx);
    EVP_PKEY *server_source_key = ed301v2_keygen(tls_libctx);
    EVP_PKEY *server_key = reload_private_key_through_pem(server_source_key);
    EVP_PKEY *client_key = ed301v2_keygen(tls_libctx);
    X509 *ca_cert = NULL;
    X509 *ca2_cert = NULL;
    ...
    TLS_OPTIONS options;
    TLS_OUTCOME outcome;
    const char *requested_group = getenv("ED301V2_TLS_GROUP");

    if (requested_group != NULL) {
        if (strcmp(requested_group, "X301") == 0) {
            tls_group_name = "X301";
            tls_group_nid = TLSEXT_nid_unknown | 0xfe30;
        } else if (strcmp(requested_group, "X301MLKEM1024") == 0) {
            tls_group_name = "X301MLKEM1024";
            tls_group_nid = TLSEXT_nid_unknown | 0xfe2f;
        } else {
    ...
    ED301V2_CHECK(deflt != NULL && v1 != NULL && pki != NULL,
        "TLS and PKI providers loaded");
    ED301V2_CHECK(ca_key != NULL && ca2_key != NULL && server_key != NULL
    ...
    ca_cert = issue_cert("Ed301-EdDSA-v2 TLS test CA", NULL, NULL, NULL, ca_key,
        ca_key, 1, 1);
    ca2_cert = issue_cert("Ed301-EdDSA-v2 untrusted CA", NULL, NULL, NULL,
        ca2_key, ca2_key, 1, 2);
    ED301V2_CHECK(ca_cert != NULL && ca2_cert != NULL, "CA certificates");
    if (ca_cert == NULL || ca2_cert == NULL)
        goto cleanup;
    server_cert = issue_cert("Ed301-EdDSA-v2 TLS server leaf", SERVER_NAME,
        "serverAuth", ca_cert, server_key, ca_key, 0, 3);
    client_cert = issue_cert("Ed301-EdDSA-v2 TLS client leaf", CLIENT_NAME,
        "clientAuth", ca_cert, client_key, ca_key, 0, 4);
    ED301V2_CHECK(server_cert != NULL && client_cert != NULL,
        "leaf certificates (CA:FALSE, digitalSignature, EKU, SAN)");
    if (server_cert == NULL || client_cert == NULL)
        goto cleanup;

    {
        SSL_CTX *mismatch = SSL_CTX_new_ex(tls_libctx, NULL, TLS_server_method());
    ...
    options.resume = 1;
    ED301V2_CHECK(run_tls(ca_cert, server_cert, server_key, NULL, NULL,
            &options, &outcome) == 1,
        "TLS run executes");
    ED301V2_CHECK(outcome.handshake_ok, "TLS 1.3 handshake completes");
    ED301V2_CHECK(outcome.group_expected, "%s key exchange negotiated", tls_group_name);
    ED301V2_CHECK(outcome.peer_cert_present
    ...
    ED301V2_CHECK(outcome.no_hrr && outcome.cv_trace_ok
    ...
    ED301V2_CHECK(outcome.application_byte == 0x42, "application data flows");
    ED301V2_CHECK(outcome.session_resumed,
     ...
     */
    {
        static const CV_EXPECT expected_server_auth[] = {
            { 'S', 1, ED301V2_TLS_SIGALG_CODE_POINT }, /* server writes CV */
            { 'C', 0, ED301V2_TLS_SIGALG_CODE_POINT }, /* client reads CV */
        };

        ED301V2_CHECK(!outcome.trace.overflow
    ...
    memset(&options, 0, sizeof(options));
    options.mutual = 1;
    ED301V2_CHECK(run_tls(ca_cert, server_cert, server_key, client_cert,
            client_key, &options, &outcome) == 1,
        "mutual TLS run executes");
    ED301V2_CHECK(outcome.handshake_ok, "mutual TLS 1.3 handshake completes");
    ED301V2_CHECK(outcome.verify_result == X509_V_OK,
        "mutual TLS chain verified");
    ED301V2_CHECK(outcome.client_peer_matches_server
        ...
        "reconstructed and verified through the provider");
    {
        static const CV_EXPECT expected_mutual[] = {
            { 'S', 1, ED301V2_TLS_SIGALG_CODE_POINT }, /* writes own CV    */
            { 'C', 0, ED301V2_TLS_SIGALG_CODE_POINT }, /* reads server CV  */
            { 'C', 1, ED301V2_TLS_SIGALG_CODE_POINT }, /* writes own CV    */
            { 'S', 0, ED301V2_TLS_SIGALG_CODE_POINT }, /* reads client CV  */
        };
     ...
     */
    {
        static const CV_EXPECT expected[] = {
            { 'C', 0, ED301V2_TLS_SIGALG_CODE_POINT },
            { 'C', 1, ED301V2_TLS_SIGALG_CODE_POINT },
        };
        CV_EVENT synthetic[4];

        memset(synthetic, 0, sizeof(synthetic));
        ED301V2_CHECK(!cv_sequence_matches(synthetic, 0, expected, 2),
            "synthetic missing sequence fails");

        synthetic[0] = (CV_EVENT){ 'C', 1,
            SSL3_MT_CERTIFICATE_VERIFY_LOCAL,
            ED301V2_TLS_SIGALG_CODE_POINT, ED301V2_SIG_BYTES,
            ED301V2_CV_WIRE_BYTES };
        synthetic[1] = (CV_EVENT){ 'C', 0,
            SSL3_MT_CERTIFICATE_VERIFY_LOCAL,
            ED301V2_TLS_SIGALG_CODE_POINT, ED301V2_SIG_BYTES,
            ED301V2_CV_WIRE_BYTES };
        ED301V2_CHECK(!cv_sequence_matches(synthetic, 2, expected, 2),
            "synthetic reordered sequence fails");

        synthetic[0] = (CV_EVENT){ 'C', 0,
            SSL3_MT_CERTIFICATE_VERIFY_LOCAL, 0x0807,
            ED301V2_SIG_BYTES, ED301V2_CV_WIRE_BYTES };
        ...
        synthetic[0] = (CV_EVENT){ 'C', 0,
            ...
            ED301V2_TLS_SIGALG_CODE_POINT, ED301V2_SIG_BYTES,
            ED301V2_CV_WIRE_BYTES };
        ED301V2_CHECK(!cv_sequence_matches(synthetic, 1, expected, 2),
            "synthetic one-direction-only sequence fails");

        synthetic[1] = synthetic[0];
        synthetic[2] = synthetic[0];
        ED301V2_CHECK(!cv_sequence_matches(synthetic, 3, expected, 2),
    ...
    /* FBL-14 negative controls, each with its expected reason. */
    {
        /* Wrong hostname. */
        memset(&options, 0, sizeof(options));
        options.expected_hostname = "wrong.v2.test.example";
        ED301V2_CHECK(run_tls(ca_cert, server_cert, server_key, NULL, NULL,
                &options, &outcome) == 1,
            "wrong-hostname run executes");
        ED301V2_CHECK(!outcome.handshake_ok
        ...
        {
            X509 *wrong_eku = issue_cert("Ed301-EdDSA-v2 wrong-EKU leaf",
                SERVER_NAME, "clientAuth", ca_cert, server_key, ca_key, 0,
                5);

            memset(&options, 0, sizeof(options));
            ED301V2_CHECK(wrong_eku != NULL
        ...
        {
            X509 *fake_ca = issue_cert("Ed301-EdDSA-v2 non-CA issuer",
                "issuer.v2.test.example", "serverAuth", NULL, ca2_key,
                ca2_key, 0, 6);
            X509 *bad_leaf = fake_ca == NULL ? NULL
                : issue_cert("Ed301-EdDSA-v2 leaf under non-CA", SERVER_NAME,
                    "serverAuth", fake_ca, server_key, ca2_key, 0, 7);

            memset(&options, 0, sizeof(options));
            ED301V2_CHECK(fake_ca != NULL && bad_leaf != NULL
        ...
        {
            X509 *foreign_leaf = issue_cert("Ed301-EdDSA-v2 foreign leaf",
                SERVER_NAME, "serverAuth", ca2_cert, server_key, ca2_key,
                0, 8);

            memset(&options, 0, sizeof(options));
            ED301V2_CHECK(foreign_leaf != NULL
        ...
        {
            X509 *corrupt = issue_cert("Ed301-EdDSA-v2 corrupt leaf",
...

    {
        X509 *default_sni_certificate = issue_cert(
            "Ed301-EdDSA-v2 default SNI leaf",
            "default.v2.test.example", "serverAuth", ca_cert,
            server_key, ca_key, 0, 10);

        ED301V2_CHECK(default_sni_certificate != NULL
    ...
    ED301V2_CHECK(!outcome.handshake_ok
        ...
        "handshake fails under TLS 1.2 (Ed301-EdDSA-v2 credentials unusable: "
    ...
    /* Capability visibility only while the provider is loaded. */
    {
        ...
        v1 = NULL;
        bare = SSL_CTX_new_ex(tls_libctx, NULL, TLS_server_method());
        ED301V2_CHECK(bare != NULL
...
cleanup:
    ED301V2_CHECK(OSSL_PROVIDER_available(NULL, ED301V2_TLS_PROVIDER) == 0,
        "v2 TLS provider was never loaded into the default context");
    ...
    if (v1 != NULL)
        OSSL_PROVIDER_unload(v1);
    OSSL_PROVIDER_unload(pki);
    OSSL_PROVIDER_unload(x_provider);
    OSSL_PROVIDER_unload(deflt);
    OSSL_LIB_CTX_free(tls_libctx);
    tls_libctx = NULL;
    return ed301v2_summary("provider_tls");
}
