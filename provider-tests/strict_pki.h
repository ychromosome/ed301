#ifndef ED301V2_STRICT_PKI_H
#define ED301V2_STRICT_PKI_H
...
static inline size_t ed301v2_bit_string_length(const ASN1_BIT_STRING *value)
{
...
static inline int ed301v2_bit_string_set(
...
static inline int ed301v2_pki_algorithm_is_exact(const X509_ALGOR *algorithm)
{
    ...
    return object != NULL
        ...
        && strcmp(text, ED301V2_OID_TEXT) == 0
        && parameter_type == V_ASN1_UNDEF
        && parameter == NULL;
}

static inline int ed301v2_pki_public_key_is_exact(const X509_PUBKEY *public_key)
{
    ...
    return public_key != NULL
        ...
        && ed301v2_pki_algorithm_is_exact(algorithm);
}

static inline int ed301v2_pki_request_is_exact(const X509_REQ *request)
{
    ...
    return signature != NULL
        && ed301v2_bit_string_length(signature) == ED301V2_SIG_BYTES
        && ed301v2_pki_algorithm_is_exact(outer)
        && ed301v2_pki_public_key_is_exact(
            X509_REQ_get_X509_PUBKEY((X509_REQ *)request));
}

static inline int ed301v2_pki_certificate_is_exact(const X509 *certificate)
{
    const ASN1_BIT_STRING *signature = NULL;
    const X509_ALGOR *outer = NULL;
    ...
    return signature != NULL
        && ed301v2_bit_string_length(signature) == ED301V2_SIG_BYTES
        && ed301v2_pki_algorithm_is_exact(outer)
        && ed301v2_pki_algorithm_is_exact(tbs)
        && X509_ALGOR_cmp(outer, tbs) == 0
        && ed301v2_pki_public_key_is_exact(
            X509_get_X509_PUBKEY((X509 *)certificate));
}

static inline int ed301v2_pki_crl_is_exact(const X509_CRL *crl)
{
    const ASN1_BIT_STRING *signature = NULL;
    const X509_ALGOR *outer = NULL;
...
#if OPENSSL_VERSION_MAJOR >= 4
    {
        ...
        if (!ed301v2_pki_algorithm_is_exact(tbs)
                || X509_ALGOR_cmp(outer, tbs) != 0)
            return 0;
    }
#endif
...
static inline int ed301v2_pki_verify_request(
    X509_REQ *request,
    EVP_PKEY *public_key)
{
    return ed301v2_pki_request_is_exact(request)
        && X509_REQ_verify(request, public_key) == 1;
}

static inline int ed301v2_pki_verify_certificate(
    X509 *certificate,
    EVP_PKEY *issuer_key)
{
    return ed301v2_pki_certificate_is_exact(certificate)
        && X509_verify(certificate, issuer_key) == 1;
}

static inline int ed301v2_pki_verify_two_certificate_chain(
    X509_STORE_CTX *store_context,
    X509 *leaf,
    X509 *trust_anchor)
{
    return store_context != NULL
        && ed301v2_pki_certificate_is_exact(leaf)
        && ed301v2_pki_certificate_is_exact(trust_anchor)
