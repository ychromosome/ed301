...
static int printed_text_contains_seed_hex(
    const char *text,
    size_t text_length,
    const unsigned char seed[ED301V2_SEED_BYTES])
{
    static const char hex[] = "0123456789abcdef";
    char compact[ED301V2_SEED_BYTES * 2];
    char colon_separated[ED301V2_SEED_BYTES * 3 - 1];
    size_t index;

    for (index = 0; index < ED301V2_SEED_BYTES; index++) {
...
static int exported_material_matches(
    ...
    int wants_private,
    int wants_public,
    const unsigned char expected_private[ED301V2_SEED_BYTES],
    const unsigned char expected_public[ED301V2_PUB_BYTES])
{
    const OSSL_PARAM *parameter;
    const OSSL_PARAM *private_parameter;
    const OSSL_PARAM *public_parameter;
    ...
    if (wants_private
            && (private_parameter->data_type != OSSL_PARAM_OCTET_STRING
                ...
                || private_parameter->data_size != ED301V2_SEED_BYTES
                || memcmp(private_parameter->data, expected_private,
                    ED301V2_SEED_BYTES) != 0))
        return 0;
    if (wants_public
            && (public_parameter->data_type != OSSL_PARAM_OCTET_STRING
                || public_parameter->data == NULL
                || public_parameter->data_size != ED301V2_PUB_BYTES
...
int main(void)
{
    ...
    /* Seed import derives the exact vector public key. */
    {
        ...
        ED301V2_CHECK(pkey != NULL
                ...
                && out_len == ED301V2_SEED_BYTES
                && memcmp(seed_out, base->seed, ED301V2_SEED_BYTES) == 0,
        ...
        /* Raw getters agree. */
        {
            ...
            ED301V2_CHECK(pkey != NULL
                    ...
                    && raw_len == ED301V2_PUB_BYTES
                    && memcmp(raw, base->public_key, ED301V2_PUB_BYTES) == 0,
                "raw public getter");
            raw_len = sizeof(raw);
            ED301V2_CHECK(pkey != NULL
                    && EVP_PKEY_get_raw_private_key(pkey, raw, &raw_len) == 1
                    && raw_len == ED301V2_SEED_BYTES
                    && memcmp(raw, base->seed, ED301V2_SEED_BYTES) == 0,
                "raw private getter");
        }
        ...
        /*
         * K6 -- Ed301-v2 public-key policy and OpenSSL
         ...
         */
        {
            ...
            EVP_PKEY *public_only = ed301v2_key_from_public(
                libctx, base->public_key, ED301V2_PUB_BYTES);
            EVP_PKEY *same_public = ed301v2_key_from_public(
                libctx, base->public_key, ED301V2_PUB_BYTES);
            EVP_PKEY *other_public = ed301v2_key_from_public(
                libctx, POSITIVE_CASES[1].public_key, ED301V2_PUB_BYTES);
            EVP_PKEY *other_pair = ed301v2_key_from_seed(
                libctx, POSITIVE_CASES[1].seed);

            ED301V2_CHECK(copy != NULL && EVP_PKEY_eq(pkey, copy) == 1,
                "duplicate matches");
            ED301V2_CHECK(public_only != NULL
                    && EVP_PKEY_eq(pkey, public_only) == 1,
                "public-only key matches the pair on the public component");
            ED301V2_CHECK(public_only != NULL && same_public != NULL
                    && EVP_PKEY_eq(public_only, same_public) == 1,
                "equal public-only keys match");
            ED301V2_CHECK(public_only != NULL && other_public != NULL
                    && EVP_PKEY_eq(public_only, other_public) == 0,
                "different public-only keys do not match");
            ED301V2_CHECK(other_pair != NULL
         ...
         */
        {
            ...
            ED301V2_CHECK(descriptor_matches(private_types, 1, 0),
                "private import descriptor is exactly one octet-string seed");
            ED301V2_CHECK(descriptor_matches(public_types, 0, 1),
                "public import descriptor is exactly one octet-string key");
            ED301V2_CHECK(descriptor_matches(keypair_types, 1, 1),
                "keypair import descriptor declares both octet strings");
            ED301V2_CHECK(from_ctx != NULL
    ...
    /* Public-only import and validation. */
    {
        ...
        unsigned char queried_public[ED301V2_PUB_BYTES] = { 0 };
        unsigned char queried_private[ED301V2_SEED_BYTES];
        size_t out_len = 0;

        ED301V2_CHECK(pkey != NULL, "public-only import");
        ED301V2_CHECK(pkey != NULL
                && EVP_PKEY_get_octet_string_param(pkey,
                    OSSL_PKEY_PARAM_PRIV_KEY, seed_out,
                    sizeof(seed_out), &out_len) != 1,
        ...
        ED301V2_CHECK(pkey != NULL
                ...
                && query[0].return_size == ED301V2_PUB_BYTES
    ...
    {
        EVP_PKEY *pkey = ed301v2_key_from_params(
            libctx, EVP_PKEY_PRIVATE_KEY,
            base->seed, ED301V2_SEED_BYTES, NULL, 0);
        unsigned char public_out[ED301V2_PUB_BYTES] = { 0 };
        ...
        ED301V2_CHECK(pkey != NULL
                ...
                && public_length == ED301V2_PUB_BYTES
                && memcmp(public_out, base->public_key,
                    ED301V2_PUB_BYTES) == 0,
    ...
    /*
     * K6 -- Ed301-v2 canonical-prime-subgroup policy.  Unlike generic OpenSSL
     ...
     */
    {
        ...
        for (point_index = 0;
            ...
            EVP_PKEY *invalid = ed301v2_key_from_public(
                libctx, point->encoding, point->encoding_len);

            ED301V2_CHECK(invalid == NULL,
    ...
    /* Keypair import with matching public key. */
    {
        EVP_PKEY *pkey = ed301v2_key_from_params(
            ...
            base->seed, ED301V2_SEED_BYTES,
            base->public_key, ED301V2_PUB_BYTES);

        ED301V2_CHECK(pkey != NULL, "matching keypair import");
        EVP_PKEY_free(pkey);
    }
     ...
     */
    {
        EVP_PKEY *raw_private = EVP_PKEY_new_raw_private_key_ex(
            libctx, ED301V2_ALG, ED301V2_PROP, base->seed, ED301V2_SEED_BYTES);
        EVP_PKEY *raw_public = EVP_PKEY_new_raw_public_key_ex(
            libctx, ED301V2_ALG, ED301V2_PROP,
            base->public_key, ED301V2_PUB_BYTES);
        unsigned char private_input[ED301V2_SEED_BYTES + 1];
        unsigned char public_input[ED301V2_PUB_BYTES + 1];
        unsigned char output[ED301V2_SEED_BYTES];
        static const size_t invalid_lengths[] = { 0, 37, 39 };
        size_t output_length = 0;
        size_t length_index;

        memcpy(private_input, base->seed, ED301V2_SEED_BYTES);
        memcpy(public_input, base->public_key, ED301V2_PUB_BYTES);
        private_input[ED301V2_SEED_BYTES] = 0;
        public_input[ED301V2_PUB_BYTES] = 0;

        ED301V2_CHECK(raw_private != NULL
                && EVP_PKEY_get_raw_private_key(
                    raw_private, NULL, &output_length) == 1
                && output_length == ED301V2_SEED_BYTES,
        ...
        ED301V2_CHECK(raw_private != NULL
                ...
                && output_length == ED301V2_SEED_BYTES
                && memcmp(output, base->seed, ED301V2_SEED_BYTES) == 0,
        ...
        ED301V2_CHECK(raw_private != NULL
                && EVP_PKEY_get_raw_public_key(
                    raw_private, NULL, &output_length) == 1
                && output_length == ED301V2_PUB_BYTES,
        ...
        ED301V2_CHECK(raw_private != NULL
                && EVP_PKEY_get_raw_public_key(
                    raw_private, output, &output_length) == 1
                && output_length == ED301V2_PUB_BYTES
                && memcmp(output, base->public_key, ED301V2_PUB_BYTES) == 0,
            "raw-private key derives the byte-exact public key");

        output_length = 0;
        ED301V2_CHECK(raw_public != NULL
                && EVP_PKEY_get_raw_public_key(
        ...
        for (length_index = 0;
            ...
            EVP_PKEY *bad_private = EVP_PKEY_new_raw_private_key_ex(
                libctx, ED301V2_ALG, ED301V2_PROP, private_input, length);
            EVP_PKEY *bad_public = EVP_PKEY_new_raw_public_key_ex(
                libctx, ED301V2_ALG, ED301V2_PROP, public_input, length);

            ED301V2_CHECK(bad_private == NULL,
                "raw-private length %zu is rejected", length);
            ED301V2_CHECK(bad_public == NULL,
    ...
    /* Rejections. */
    {
        ...
        memcpy(long_seed, base->seed, ED301V2_SEED_BYTES);
        long_seed[38] = 0;

        /*
         * K7 -- Provider atomic-keypair contract: a seed plus a different,
         * individually valid v2 public key is rejected during import, so
         * no inconsistent EVP_PKEY can reach EVP_PKEY_check().
         */
        bad = ed301v2_key_from_params(libctx, EVP_PKEY_KEYPAIR,
            base->seed, ED301V2_SEED_BYTES, wrong_public, ED301V2_PUB_BYTES);
        ED301V2_CHECK(bad == NULL, "mismatched keypair import is rejected");
        ERR_clear_error();

        bad = ed301v2_key_from_params(libctx, EVP_PKEY_KEYPAIR,
            short_seed, sizeof(short_seed), NULL, 0);
        ED301V2_CHECK(bad == NULL, "37-byte seed is rejected");
        ERR_clear_error();

        bad = ed301v2_key_from_params(libctx, EVP_PKEY_KEYPAIR,
            long_seed, sizeof(long_seed), NULL, 0);
        ED301V2_CHECK(bad == NULL, "39-byte seed is rejected");
        ERR_clear_error();

        bad = ed301v2_key_from_public(libctx, base->public_key, 37);
        ED301V2_CHECK(bad == NULL, "37-byte public key is rejected");
        ERR_clear_error();

        bad = ed301v2_key_from_public(libctx, base->public_key, 39);
        ED301V2_CHECK(bad == NULL,
         ...
         */
        bad = ed301v2_key_from_params(libctx, EVP_PKEY_KEYPAIR,
            NULL, 0, base->public_key, ED301V2_PUB_BYTES);
        ED301V2_CHECK(bad != NULL,
        ...
        if (bad != NULL) {
            ...
            ED301V2_CHECK(EVP_PKEY_get_octet_string_param(bad,
                    OSSL_PKEY_PARAM_PRIV_KEY, seed_out,
                    sizeof(seed_out), &out_len) != 1,
        ...
        bad = ed301v2_key_from_params(libctx, EVP_PKEY_KEYPAIR,
            NULL, 0, NULL, 0);
        ED301V2_CHECK(bad == NULL, "empty import is rejected");
        ERR_clear_error();
        ...
        /* Malformed parameter type: UTF8 string instead of octets. */
        {
            ...
            params[0] = OSSL_PARAM_construct_utf8_string(
                OSSL_PKEY_PARAM_PUB_KEY,
                (char *)base->public_key, ED301V2_PUB_BYTES);
     ...
     */
    {
        ...
        EVP_PKEY_CTX *ctx = EVP_PKEY_CTX_new_from_name(
            libctx, ED301V2_ALG, ED301V2_PROP);
        unsigned char wrong_public[ED301V2_PUB_BYTES];
        unsigned char seed_out[ED301V2_SEED_BYTES] = { 0 };
        unsigned char public_out[ED301V2_PUB_BYTES] = { 0 };
        ...
        ED301V2_CHECK(pkey != NULL
                && ed301v2_digest_sign(libctx, pkey, base->message,
    ...
    /* Encoded-public setter policy. */
    {
        ...
        ED301V2_CHECK(pkey != NULL
                && EVP_PKEY_set1_encoded_public_key(pkey,
                    base->public_key, ED301V2_PUB_BYTES) == 1,
            "matching encoded public key is accepted");
        ED301V2_CHECK(pkey != NULL
                && EVP_PKEY_set1_encoded_public_key(pkey,
                    different, ED301V2_PUB_BYTES) != 1,
    ...
    {
        EVP_PKEY *generated = ed301v2_keygen(libctx);
        EVP_PKEY *reimported = NULL;
        unsigned char seed_out[38];
        unsigned char public_out[38];
        unsigned char sig[76];
        size_t out_len = 0;
        static const unsigned char probe[] = "Ed301-EdDSA keygen probe";

        ED301V2_CHECK(generated != NULL, "key generation");
        if (generated != NULL) {
            EVP_PKEY_CTX *check_ctx = EVP_PKEY_CTX_new_from_pkey(
                libctx, generated, ED301V2_PROP);
            ...
            ED301V2_CHECK(EVP_PKEY_get_octet_string_param(generated,
                    OSSL_PKEY_PARAM_PRIV_KEY, seed_out,
                    sizeof(seed_out), &out_len) == 1
                    && out_len == ED301V2_SEED_BYTES
            ...
            reimported = ed301v2_key_from_seed(libctx, seed_out);
            ED301V2_CHECK(reimported != NULL
                    && EVP_PKEY_eq(generated, reimported) == 1,
                "generated seed reimports to an equal key");

            ED301V2_CHECK(ed301v2_digest_sign(libctx, generated, probe,
                    sizeof(probe) - 1, sig)
                    && ed301v2_digest_verify(libctx, generated, probe,
    ...
    {
        EVP_PKEY *first = ed301v2_keygen(libctx);
        EVP_PKEY *second = ed301v2_keygen(libctx);

        ED301V2_CHECK(first != NULL && second != NULL
    ...
    return ed301v2_summary("provider_keymgmt");
}
