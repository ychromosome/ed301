...
#define ED301_NAME "Ed301-EdDSA"
#define ED301_PROPERTIES "provider=ed301_eddsa_v2"
    ...
    size_t *operation_errors)
{
    ...
    *maximum_t = 0;
    for (index = 0; index < batches; index++) {
        dudect_state_t current = dudect_main(&context);
    ...
    if (context.ttest_ctxs[0]->n[0] <= DUDECT_ENOUGH_MEASUREMENTS
