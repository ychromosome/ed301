    ...
    size_t *out_len)
{
    OSSL_ENCODER_CTX *ctx = OSSL_ENCODER_CTX_new_for_pkey(
        pkey, selection, format, structure, ED301V2_PKI_PROP);
    ...
    const unsigned char seed[ED301V2_SEED_BYTES])
{
    ...
    if (libctx == NULL
            || (pki = OSSL_PROVIDER_load(
                    libctx, ED301V2_PKI_PROVIDER)) == NULL)
        goto done;
    key = ed301v2_key_from_seed(libctx, seed);
    encoder = key == NULL ? NULL : OSSL_ENCODER_CTX_new_for_pkey(
        key, OSSL_KEYMGMT_SELECT_PRIVATE_KEY, "DER",
        "EncryptedPrivateKeyInfo", ED301V2_PKI_PROP);
    if (encoder != NULL)
    ...
    result = encoder != NULL
        ...
        && data != NULL && data_length > ED301V2_PKCS8_DER_BYTES;
    ...
    const unsigned char seed[ED301V2_SEED_BYTES])
{
    ...
    if (libctx == NULL
            ...
            || (pki = OSSL_PROVIDER_load(
                    libctx, ED301V2_PKI_PROVIDER)) == NULL
            || (key = ed301v2_key_from_seed(libctx, seed)) == NULL)
        goto done;
    worker.key = key;
    ...
    int selection)
{
    ...
    if (strcmp(format, "DER") == 0)
        return ed301v2_strict_der_import(libctx, data, data_len, is_public);
    if (strcmp(format, "PEM") == 0)
        return ed301v2_strict_pem_import(libctx, data, data_len, is_public);
    return NULL;
}
...
/*
 * Construct the small Ed301-EdDSA-v2 PrivateKeyInfo grammar from semantic fields.
    ...
    size_t seed_length)
{
    ...
    output[index++] = 0x30;
    output[index++] = 0x0d;
    memcpy(output + index, ED301V2_PKCS8_PREFIX + 7, ED301V2_OID_TLV_BYTES);
    index += ED301V2_OID_TLV_BYTES;
    output[index++] = 0x04;
...
int main(void)
{
    ...
    OSSL_PROVIDER *v1;
#ifdef X301_CODEC_TEST
    const unsigned char *base_seed = SECRET_A;
    const unsigned char *base_public = PUBLIC_A;
#else
    const unsigned char *base_seed = POSITIVE_CASES[0].seed;
    const unsigned char *base_public = POSITIVE_CASES[0].public_key;
#endif
    EVP_PKEY *pkey;

    ed301v2_property = ED301V2_PKI_PROP;
    v1 = ed301v2_load_named(libctx, &deflt, ED301V2_PKI_PROVIDER);
    pkey = ed301v2_key_from_seed(libctx, base_seed);
    ED301V2_CHECK(v1 != NULL && pkey != NULL, "provider and key");
    ED301V2_CHECK(pki_first_load_has_no_cipher_fallback(base_seed),
        "PKI-first load cannot auto-load a child cipher provider and "
        "recovers after the application loads default");
    ED301V2_CHECK(encrypted_encode_survives_worker_teardown(base_seed),
     ...
     */
    ED301V2_CHECK(pkey != NULL && der_pem_der_is_identical(libctx, pkey,
    ...
    /* PKCS#8 DER is byte-exact and round-trips. */
    {
        ...
        ED301V2_CHECK(der != NULL && der_len == ED301V2_PKCS8_DER_BYTES
                && memcmp(der, ED301V2_PKCS8_PREFIX,
                    sizeof(ED301V2_PKCS8_PREFIX)) == 0
                && memcmp(der + sizeof(ED301V2_PKCS8_PREFIX), base_seed,
                    38) == 0,
        ...
        if (der != NULL && der_len == ED301V2_PKCS8_DER_BYTES) {
             ...
             */
            ED301V2_CHECK(all_truncations_rejected(libctx, der, der_len,
            ...
            ED301V2_CHECK(decode(libctx, mutated, der_len + 1, "DER",
                    "PrivateKeyInfo",
            ...
            ED301V2_CHECK(decode(libctx, nonminimal, der_len + 1, "DER",
                    "PrivateKeyInfo",
                    OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
            ...
            ED301V2_CHECK(decode(libctx, indefinite, der_len + 2, "DER",
                    "PrivateKeyInfo",
                    OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
            ...
            memcpy(mutated, der, der_len);
            mutated[10] ^= 1;
            ED301V2_CHECK(decode(libctx, mutated, der_len, "DER",
                    "PrivateKeyInfo",
                    OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
...

            {
                ...
                ERR_clear_error();
                ED301V2_CHECK(long_seed_der_length
                        == ED301V2_PKCS8_DER_BYTES + 1
     ...
     */
    {
        unsigned char with_null[ED301V2_PKCS8_DER_BYTES + 2];
        size_t index = 0;
        ...
        memcpy(with_null + index, ED301V2_PKCS8_PREFIX + 7,
            ED301V2_OID_TLV_BYTES);
        index += ED301V2_OID_TLV_BYTES;
        ...
        memcpy(with_null + index, base_seed, 38);
        index += 38;
        ED301V2_CHECK(index == sizeof(with_null), "NULL-variant layout");
        ED301V2_CHECK(decode(libctx, with_null, sizeof(with_null), "DER",
                "PrivateKeyInfo",
                OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
     ...
     * only PrivateKeyInfo with version INTEGER 0 is part of
    ...
    */
    {
        unsigned char version_one[ED301V2_PKCS8_DER_BYTES] = {0};
        unsigned char one_asymmetric_key[128] = {0};
        size_t private_key_info_length;
        size_t index = 0;

        private_key_info_length = make_pkcs8_with_seed_length(
            version_one, sizeof(version_one), base_seed, ED301V2_SEED_BYTES);
        ED301V2_CHECK(private_key_info_length == sizeof(version_one),
        ...
        memcpy(one_asymmetric_key + index, version_one + 2,
            ED301V2_PKCS8_DER_BYTES - 2);
        index += ED301V2_PKCS8_DER_BYTES - 2;
        ...
        memcpy(one_asymmetric_key + index,
            base_public, ED301V2_PUB_BYTES);
        index += ED301V2_PUB_BYTES;
        ED301V2_CHECK(index == 103, "OneAsymmetricKey v2 layout");
        ED301V2_CHECK(decode(libctx, one_asymmetric_key, index, "DER",
                "PrivateKeyInfo",
                OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
    ...
    {
        unsigned char historical_p8[ED301V2_PKCS8_DER_BYTES];
        unsigned char historical_spki[ED301V2_SPKI_DER_BYTES];
        ...
        memcpy(historical_p8 + sizeof(HISTORICAL_PKCS8_PREFIX),
            base_seed, 38);
        ED301V2_CHECK(decode(libctx, historical_p8, sizeof(historical_p8),
                "DER", "PrivateKeyInfo",
                OSSL_KEYMGMT_SELECT_PRIVATE_KEY) == NULL,
        ...
        memcpy(historical_spki + sizeof(HISTORICAL_SPKI_PREFIX),
            base_public, 38);
        ED301V2_CHECK(decode(libctx, historical_spki,
    ...
    {
        unsigned char x301_p8[ED301V2_PKCS8_DER_BYTES];
        unsigned char x301_spki[ED301V2_SPKI_DER_BYTES];

        memcpy(x301_p8, X301_PKCS8_PREFIX, sizeof(X301_PKCS8_PREFIX));
        memcpy(x301_p8 + sizeof(X301_PKCS8_PREFIX), base_seed, 38);
        ED301V2_CHECK(decode(libctx, x301_p8, sizeof(x301_p8), "DER",
        ...
        memcpy(x301_spki + sizeof(X301_SPKI_PREFIX),
            base_public, 38);
        ED301V2_CHECK(decode(libctx, x301_spki, sizeof(x301_spki), "DER",
    ...
    /* SPKI DER is byte-exact and round-trips. */
    {
        ...
        ED301V2_CHECK(der != NULL && der_len == ED301V2_SPKI_DER_BYTES
                && memcmp(der, ED301V2_SPKI_PREFIX,
                    sizeof(ED301V2_SPKI_PREFIX)) == 0
                && memcmp(der + sizeof(ED301V2_SPKI_PREFIX), base_public,
                    38) == 0,
        ...
        if (der != NULL && der_len == ED301V2_SPKI_DER_BYTES) {
            unsigned char malformed[80];
            unsigned char nonminimal[80];
            unsigned char indefinite[80];
            static const size_t truncation_boundaries[] = {
                0, 1, 2, 3, 4, 5, 6, 17, 18, 19, 20,
                ED301V2_SPKI_DER_BYTES - 1
            };

            memcpy(malformed, der, der_len);
            memcpy(malformed + sizeof(ED301V2_SPKI_PREFIX),
#ifdef X301_CODEC_TEST
                FIELD_MODULUS, 38);
#else
                POINT_CASES[2].encoding, 38); /* identity */
#endif
            ED301V2_CHECK(decode(libctx, malformed, der_len, "DER",
                    "SubjectPublicKeyInfo",
                    OSSL_KEYMGMT_SELECT_PUBLIC_KEY) == NULL,
                "SPKI with invalid public material is rejected");
            ERR_clear_error();
            ...
            ED301V2_CHECK(decode(libctx, malformed, der_len + 1, "DER",
                    "SubjectPublicKeyInfo",
                    OSSL_KEYMGMT_SELECT_PUBLIC_KEY) == NULL,
    ...
    {
        unsigned char with_null[ED301V2_SPKI_DER_BYTES + 2];
        size_t index = 0;
        ...
        with_null[index++] = 0x30;
        with_null[index++] = 0x0f;
        memcpy(with_null + index, ED301V2_SPKI_PREFIX + 4,
            ED301V2_OID_TLV_BYTES);
        index += ED301V2_OID_TLV_BYTES;
        ...
        with_null[index++] = 0x00;
        memcpy(with_null + index, base_public, ED301V2_PUB_BYTES);
        index += ED301V2_PUB_BYTES;
        ED301V2_CHECK(index == sizeof(with_null), "SPKI NULL-variant layout");
        ED301V2_CHECK(decode(libctx, with_null, sizeof(with_null), "DER",
                "SubjectPublicKeyInfo",
                OSSL_KEYMGMT_SELECT_PUBLIC_KEY) == NULL,
    ...
    /* PEM round trips. */
    {
        ...
        ED301V2_CHECK(pem != NULL && pem_len >= 27
        ...
        ED301V2_CHECK(pem != NULL && pem_len >= 26
    ...
    /* The PKI-only artifact exposes no text/hex encoder (F5). */
    {
        ...
        ED301V2_CHECK(text == NULL && text_len == 0,
    ...
    /* OpenSSL Ed448 pattern: direct encrypted PKCS#8 in DER and PEM. */
    {
        OSSL_ENCODER_CTX *ctx = OSSL_ENCODER_CTX_new_for_pkey(
            ...
            OSSL_KEYMGMT_SELECT_PRIVATE_KEY
                | OSSL_KEYMGMT_SELECT_PUBLIC_KEY,
            "DER", "PrivateKeyInfo", ED301V2_PKI_PROP);
        ...
        if (decrypted != NULL)
            plain_der_length = i2d_PKCS8_PRIV_KEY_INFO(
                decrypted, &plain_der);
        ED301V2_CHECK(ctx != NULL && set_ok && encode_result == 1
                && data != NULL && data_len > ED301V2_PKCS8_DER_BYTES
                && encrypted != NULL && cursor == data + data_len
                && decrypted != NULL && wrong_password == NULL
                && plain_der_length == ED301V2_PKCS8_DER_BYTES
                && memcmp(plain_der, ED301V2_PKCS8_PREFIX,
                    sizeof(ED301V2_PKCS8_PREFIX)) == 0
                && memcmp(plain_der + sizeof(ED301V2_PKCS8_PREFIX),
                    base_seed, ED301V2_SEED_BYTES) == 0,
            "direct encrypted PKCS#8 DER round trip and wrong-password "
            "rejection");
        ED301V2_CHECK(second_data != NULL && second_data_len > 0
...

    {
        ...
        ED301V2_CHECK(ctx != NULL && encode_result != 1
...

    {
        ...
        ED301V2_CHECK(encode_result == 1 && data != NULL
                && data_len == ED301V2_PKCS8_DER_BYTES
                && memcmp(data, ED301V2_PKCS8_PREFIX,
                    sizeof(ED301V2_PKCS8_PREFIX)) == 0
                && memcmp(data + sizeof(ED301V2_PKCS8_PREFIX),
                    base_seed, ED301V2_SEED_BYTES) == 0,
    ...
    OSSL_PROVIDER_unload(deflt);
    OSSL_LIB_CTX_free(libctx);
    return ed301v2_summary("provider_serialization");
}
