...
{
    return setenv("ED301_EDDSA_V2_ALLOC_FAILPOINT", site, 1) == 0;
}

static void clear_alloc_failpoint(void)
{
    unsetenv("ED301_EDDSA_V2_ALLOC_FAILPOINT");
}
...
static int full_cycle(void)
{
    ...
    if (v2 == NULL)
        goto done;
    pkey = ed301v2_key_from_seed(libctx, POSITIVE_CASES[0].seed);
    if (pkey == NULL)
        goto done;
    if (!ed301v2_digest_sign(libctx, pkey, POSITIVE_CASES[0].message,
...
static int allocation_fail_key_import(OSSL_LIB_CTX *libctx)
{
    ...
    if (set_alloc_failpoint("key_import")) {
        pkey = ed301v2_key_from_public(libctx, POSITIVE_CASES[0].public_key,
            ED301V2_PUB_BYTES);
        failed = pkey == NULL;
    }
    clear_alloc_failpoint();
...
{
    EVP_PKEY *pkey = ed301v2_key_from_public(libctx,
    ...
    if (set_alloc_failpoint("key_set_encoded_public")) {
        failed = EVP_PKEY_set1_encoded_public_key(pkey,
            POSITIVE_CASES[0].public_key, ED301V2_PUB_BYTES) != 1;
    }
    clear_alloc_failpoint();
    failed = failed
...
static int allocation_fail_signature_new(OSSL_LIB_CTX *libctx)
{
    ...
    pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_FAILPOINT_PROP);
    failed = failed
        && pctx != NULL
        && ed301v2_sign_message_init(libctx, pctx, NULL)
...
static int allocation_fail_signature_duplicate(OSSL_LIB_CTX *libctx)
{
    ...
    if (mctx == NULL
            || EVP_DigestSignInit_ex(mctx, NULL, NULL, libctx,
                ED301V2_FAILPOINT_PROP, pkey, NULL) != 1)
        goto done;
...
int main(void)
{
    const int rust_alloc_only =
        getenv("ED301V2_RUST_ALLOC_ONLY") != NULL;
    ...
    ED301V2_CHECK(full_cycle(), "baseline cycle under the counting allocator");
    ...
    /* Injected Rust panics fail closed across the C ABI boundary. */
    {
         ...
         */
        v2 = ed301v2_load_named(libctx, &deflt, ED301V2_FAILPOINT_PROVIDER);
        ed301v2_property = ED301V2_FAILPOINT_PROP;
        ED301V2_CHECK(v2 != NULL, "failpoint artifact for panic tests");

        /* The separately named failpoint artifact has no TLS capability. */
        ED301V2_CHECK(v2 != NULL
                && OSSL_PROVIDER_get_capabilities(v2, "TLS-SIGALG",
        ...
        ED301V2_CHECK(allocation_fail_key_new(libctx),
            "key_new allocation failpoint returns null and recovers");
        ED301V2_CHECK(allocation_fail_key_generate(libctx),
            "key_generate allocation failpoint returns null and recovers");
        ED301V2_CHECK(allocation_fail_key_import(libctx),
            "key_import shared-state allocation fails closed and recovers");
        ED301V2_CHECK(allocation_fail_key_set_encoded_public(libctx),
            "encoded-public shared-state allocation fails closed and recovers");
        ED301V2_CHECK(allocation_fail_key_duplicate(libctx),
            "key_duplicate allocation failpoint returns null and recovers");
        ED301V2_CHECK(allocation_fail_signature_new(libctx),
        ...
        for (index = 0;
            ...
            if (strcmp(cases[index].failpoint, "key_generate") == 0) {
                ...
                unsetenv("ED301_EDDSA_V2_PANIC_FAILPOINT");
                pkey = ed301v2_key_from_seed(libctx,
                    POSITIVE_CASES[0].seed);
                setenv("ED301_EDDSA_V2_PANIC_FAILPOINT",
                    cases[index].failpoint, 1);
                failed_closed = pkey != NULL
                    && !ed301v2_digest_sign(libctx, pkey,
     ...
     */
    {
        OSSL_LIB_CTX *libctx = OSSL_LIB_CTX_new();
        OSSL_PROVIDER *deflt = NULL;
        OSSL_PROVIDER *v2 = ed301v2_load(libctx, &deflt);
        EVP_PKEY *pkey;
        unsigned char sig[76];

        setenv("ED301_EDDSA_V2_PANIC_FAILPOINT", "signature_sign",
            1);
        setenv("ED301_EDDSA_V2_ALLOC_FAILPOINT", "signature_new",
            1);
        pkey = ed301v2_key_from_seed(libctx, POSITIVE_CASES[0].seed);
        ED301V2_CHECK(v2 != NULL && pkey != NULL
                && ed301v2_digest_sign(libctx, pkey,
                    POSITIVE_CASES[0].message,
                    POSITIVE_CASES[0].message_len, sig)
                && memcmp(sig, POSITIVE_CASES[0].signature,
    ...
    /* Duplicate/free/unload ordering. */
    {
        ...
        OSSL_PROVIDER *second_handle =
            OSSL_PROVIDER_load(libctx, ED301V2_PROVIDER);
        EVP_PKEY *pkey = ed301v2_key_from_seed(libctx,
        ...
        sig_len = sizeof(first_sig);
        ED301V2_CHECK(copy != NULL
                && ed301v2_digest_sign(libctx, copy,
                    POSITIVE_CASES[1].message,
    ...
    return ed301v2_summary("provider_hardening");
}
