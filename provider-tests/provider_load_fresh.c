...
static void *fresh_worker_main(void *argument)
{
    ...
    v2 = ed301v2_load_named(libctx, NULL, ED301V2_PKI_PROVIDER);
    if (worker->expect_success) {
        if (deflt == NULL || v2 == NULL
                || !ed301v2_queue_is_sentinel_only()) {
            worker->failed = 1;
            goto done;
        }
        fetched = EVP_SIGNATURE_fetch(libctx, ED301V2_ALG, ED301V2_PKI_PROP);
        if (fetched == NULL)
            worker->failed = 1;
    } else {
        if (v2 != NULL || !ed301v2_queue_is_sentinel_only())
            worker->failed = 1;
    }
    ...
    const char *dir_b)
{
    ...
    if (strcmp(mode, "conflict") == 0) {
        ...
        if (deflt == NULL
                || OBJ_create(ED301V2_OID_TEXT, "ED301V2-FRESH-COLLIDER",
                    "ED301V2 fresh conflict object") == NID_undef) {
    ...
    if (expect_success && !ed301v2_registry_is_exact()) {
...
int main(int argc, char **argv)
{
    ...
    const char *dir_b = getenv("ED301V2_FRESH_COPY_DIR");
    char self[4096];
    ssize_t self_length;
    int repeat;
    ...
    if (modules == NULL || dir_b == NULL) {
        fprintf(stderr,
            "OPENSSL_MODULES and ED301V2_FRESH_COPY_DIR are required\n");
        return 2;
    }
    ...
    for (repeat = 0; repeat < FRESH_REPEATS; repeat++) {
        ED301V2_CHECK(spawn_child(self, "same-dso", modules, dir_b) == 0,
            "fresh-process same-DSO first-load race (round %d)", repeat);
        ED301V2_CHECK(spawn_child(self, "separate-copy", modules, dir_b) == 0,
            "fresh-process separate-copy first-load race (round %d)",
            repeat);
    }
    ED301V2_CHECK(spawn_child(self, "conflict", modules, dir_b) == 0,
        "host preflight blocks a conflicting fresh-process registry");

    return ed301v2_summary("provider_load_fresh");
}
