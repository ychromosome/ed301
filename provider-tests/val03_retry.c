...
static void *competitor_main(void *argument)
{
    ...
    if (competitor->conflicting) {
        nid = OBJ_create(ED301V2_OID_TEXT, "ED301V2-VAL03-COLLIDER",
            "ED301V2 val03 conflict object");
    ...
    long delay_milliseconds, int conflicting)
{
    ...
    first_queue_ok = ed301v2_queue_is_sentinel_only();
    pthread_join(thread, NULL);
    ...
    ed301v2_seed_error_sentinel();
    retry = ed301v2_load_named(libctx, NULL, ED301V2_PKI_PROVIDER);
    printf("%s: unsafe first load %s in %.3f s; explicit retry %s\n",
        label, v2 == NULL ? "blocked" : "unexpectedly succeeded",
        elapsed, retry != NULL ? "succeeded" : "blocked");
    lane_ok = v2 == NULL && first_queue_ok && elapsed <= 1.0
        && ed301v2_queue_is_sentinel_only()
        && (conflicting
            ? retry == NULL
            : retry != NULL && ed301v2_registry_is_exact());
    if (!lane_ok)
...
int main(int argc, char **argv)
{
    ...
    return ed301v2_summary("val03_retry");
}
