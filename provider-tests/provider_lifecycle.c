...
static int lifecycle_key_after_handle_release(void)
{
    ...
    if (v2 == NULL)
        goto done;
    pkey = ed301v2_key_from_seed(libctx, POSITIVE_CASES[1].seed);
    if (pkey == NULL)
        goto done;
     ...
     */
    handles_released = OSSL_PROVIDER_unload(v2) == 1;
    v2 = NULL;
...
static int lifecycle_two_libctx(void)
{
    ...
    OSSL_PROVIDER *first_v2 = NULL;
    OSSL_PROVIDER *second_v2 = NULL;
    ...
    if (first == NULL || second == NULL)
        goto done;
    first_v2 = ed301v2_load(first, &first_deflt);
    second_v2 = ed301v2_load(second, &second_deflt);
    if (first_v2 == NULL || second_v2 == NULL)
        goto done;
    first_key = ed301v2_key_from_seed(first, POSITIVE_CASES[0].seed);
    second_key = ed301v2_key_from_seed(second, POSITIVE_CASES[1].seed);
    ...
    second_key = NULL;
    OSSL_PROVIDER_unload(second_v2);
    second_v2 = NULL;
...
static int lifecycle_sign_once(
    ...
    unsigned char signature[ED301V2_SIG_BYTES])
{
    size_t signature_length = ED301V2_SIG_BYTES;
    ...
    return context != NULL && test_case != NULL
        ...
        && memcmp(signature, test_case->signature, ED301V2_SIG_BYTES) == 0;
}
    ...
    const POSITIVE_CASE *test_case)
{
    return context != NULL && test_case != NULL
        && EVP_PKEY_verify(context,
            test_case->signature, ED301V2_SIG_BYTES,
            test_case->message, test_case->message_len) == 1;
}
    ...
    int free_original_first)
{
    ...
    unsigned char original_signature[ED301V2_SIG_BYTES];
    unsigned char duplicate_signature[ED301V2_SIG_BYTES];
    int first_ok;
    int second_ok;
    int ok = 0;
    const char *failure_stage = "sign context creation";

    sign_original = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
    if (sign_original == NULL
            || !ed301v2_sign_message_init(libctx, sign_original, NULL))
        goto done;
    ...
    verify_original = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
    if (verify_original == NULL
            || !ed301v2_verify_message_init(libctx, verify_original, NULL))
        goto done;
...
int main(void)
{
     ...
     */
    ED301V2_CHECK(lifecycle_key_after_handle_release(),
     ...
     */
    ED301V2_CHECK(lifecycle_two_libctx(),
    ...
    if (libctx != NULL)
        v2 = ed301v2_load(libctx, &deflt);
    if (v2 != NULL)
        shared_key = ed301v2_key_from_seed(libctx, POSITIVE_CASES[3].seed);
    ED301V2_CHECK(libctx != NULL && v2 != NULL && shared_key != NULL,
     ...
     * state to clone because Ed301-EdDSA-v2 is pure one-shot EdDSA.
     */
    ED301V2_CHECK(shared_key != NULL
    ...
    OSSL_PROVIDER_unload(deflt);
    OSSL_LIB_CTX_free(libctx);
    return ed301v2_summary("provider_lifecycle");
}
