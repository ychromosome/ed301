/*
 * Context parameter-state contract inherited from the bound v1 provider.
 * Exact v2 context KATs are additionally enforced by provider_signature.c.
 */
...
static int sign_message(OSSL_LIB_CTX *libctx, EVP_PKEY *pkey,
    ...
    int set_after_init, unsigned char signature[ED301V2_SIG_BYTES])
{
    EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
    OSSL_PARAM params[2];
    unsigned char empty = 0;
    size_t signature_len = ED301V2_SIG_BYTES;
    int ok;
...
static int verify_message(OSSL_LIB_CTX *libctx, EVP_PKEY *pkey,
    const unsigned char *message, size_t message_len,
    int context_present, const unsigned char *context, size_t context_len,
    const unsigned char signature[ED301V2_SIG_BYTES])
{
    EVP_PKEY_CTX *pctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey, ED301V2_PROP);
    OSSL_PARAM params[2];
    unsigned char empty = 0;
    ...
    if (ed301v2_verify_message_init(
            libctx, pctx, context_present ? params : NULL))
        result = EVP_PKEY_verify(pctx, signature, ED301V2_SIG_BYTES,
            message, message_len);
    EVP_PKEY_CTX_free(pctx);
    ERR_clear_error();
    return result;
    ...
    unsigned char signature[ED301V2_SIG_BYTES])
{
    ...
    params[1] = OSSL_PARAM_construct_end();
    ok = EVP_DigestSignInit_ex(mctx, NULL, NULL, libctx, ED301V2_PROP,
             pkey, params) == 1
    ...
    const unsigned char signature[ED301V2_SIG_BYTES])
{
    ...
    params[0] = OSSL_PARAM_construct_octet_string(
        OSSL_SIGNATURE_PARAM_CONTEXT_STRING, (void *)context, context_len);
    params[1] = OSSL_PARAM_construct_end();
    if (EVP_DigestVerifyInit_ex(mctx, NULL, NULL, libctx, ED301V2_PROP,
            pkey, params) == 1)
        result = EVP_DigestVerify(mctx, signature, ED301V2_SIG_BYTES,
            message, message_len);
    EVP_MD_CTX_free(mctx);
    ERR_clear_error();
    return result;
...
int main(void)
{
    ...
    unsigned char no_context[ED301V2_SIG_BYTES] = { 0 };
    unsigned char empty_init[ED301V2_SIG_BYTES] = { 0 };
    unsigned char empty_set[ED301V2_SIG_BYTES] = { 0 };
    unsigned char alpha_init[ED301V2_SIG_BYTES] = { 0 };
    unsigned char alpha_set[ED301V2_SIG_BYTES] = { 0 };
    unsigned char alpha_repeat[ED301V2_SIG_BYTES] = { 0 };
    unsigned char alpha_digest[ED301V2_SIG_BYTES] = { 0 };
    unsigned char beta_init[ED301V2_SIG_BYTES] = { 0 };
    unsigned char binary_init[ED301V2_SIG_BYTES] = { 0 };
    unsigned char max_init[ED301V2_SIG_BYTES] = { 0 };
    unsigned char max_context[255];
    unsigned char overlong_context[256];
    unsigned char rejected_output[ED301V2_SIG_BYTES];
    OSSL_LIB_CTX *libctx;
    ...
    provider = ed301v2_load(libctx, &deflt);
    pkey = provider == NULL ? NULL : ed301v2_key_from_seed(libctx, seed);
    ED301V2_CHECK(provider != NULL, "provider load");
    ED301V2_CHECK(pkey != NULL, "fixed key import");
    if (pkey == NULL)
        goto done;

    ED301V2_CHECK(sign_message(libctx, pkey, message, sizeof(message),
    ...
    ED301V2_CHECK(memcmp(no_context, empty_init, ED301V2_SIG_BYTES) == 0
            && memcmp(no_context, empty_set, ED301V2_SIG_BYTES) == 0,
    ...
    ED301V2_CHECK(sign_digest(libctx, pkey, message, sizeof(message),
            alpha, sizeof(alpha) - 1, alpha_digest),
        "alpha context signs through DigestSign");
    ED301V2_CHECK(memcmp(alpha_init, alpha_set, ED301V2_SIG_BYTES) == 0
            && memcmp(alpha_init, alpha_repeat, ED301V2_SIG_BYTES) == 0
            && memcmp(alpha_init, alpha_digest, ED301V2_SIG_BYTES) == 0,
    ...
    ED301V2_CHECK(memcmp(no_context, alpha_init, ED301V2_SIG_BYTES) != 0
            && memcmp(alpha_init, beta_init, ED301V2_SIG_BYTES) != 0
            && memcmp(alpha_init, binary_init, ED301V2_SIG_BYTES) != 0
            && memcmp(alpha_init, max_init, ED301V2_SIG_BYTES) != 0,
        "distinct contexts produce distinct signatures");

    ED301V2_CHECK(verify_message(libctx, pkey, message, sizeof(message),
    ...
    return ed301v2_summary("provider_context_contract");
}
