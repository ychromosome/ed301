\\ Independent security-parameter audit for an ED301-v2 candidate (adapted from the v1 audit script; candidate from environment).
...
default(timer, 0);
\\ v2: abort the whole run on the first failed must(); gp otherwise recovers and continues,
\\ which let a failed v1-rule run print a pass marker on 2026-09-09.
default(recover, 0);
...
\\ Candidate under audit: taken from the environment so that the template is never bound to
\\ a particular family. Required: ED301_D, ED301_C, ED301_Q, ED301_QT (decimal strings).
p = 2^301 - 2^89 + 907;
d = eval(getenv("ED301_D"));
c = eval(getenv("ED301_C"));
s = 907 + c;
a = lift(Mod(s, p)^2);
q = eval(getenv("ED301_Q"));
qt = eval(getenv("ED301_QT"));
N = 4 * q;
Nt = 4 * qt;

must(d != 0 && c >= 0, "candidate environment incomplete");
\\ Criterion 5 of the rule file: main subgroup exactly 300 bits (4q >= 2^301), twist >= 299 bits.
must(logint(q, 2) + 1 == 300 && 4 * q >= 2^301, "main subgroup order does not have 300 bits");
must(logint(qt, 2) + 1 >= 299, "twist subgroup order below 299 bits");
output_path = Str(getenv("ED301_OUT"), "/audit_v3_d", d, "_c", c, "_pari.txt");
...
\\ v2: witnesses for the exact order k (not for q-1): for every prime r | k, p^(k/r) != 1.
k_q = znorder(Mod(p, q), F_q_minus_1);
k_qt = znorder(Mod(p, qt), F_qt_minus_1);
W_q = order_witnesses(p, q, k_q, factor(k_q), "ord_q(p)");
W_qt = order_witnesses(p, qt, k_qt, factor(k_qt), "ord_qt(p)");
\\ v2: the v1 acceptance rule (Pruefvektor 5) is "no embedding degree k <= 100"; maximality was a v1
\\ observation, not a requirement (Ed25519: (l-1)/6, Ed448: (l-1)/2). Enforce exactly the search rule
\\ (k > 100); the SafeCurves-style floor k >= (order-1)/100 is reported below, not enforced.
must(k_q > 100 && k_qt > 100, "small embedding degree");
\\ SafeCurves-style floor, reported only (not part of the search rule):
safecurves_floor_pass = (k_q >= (q - 1) / 100 && k_qt >= (qt - 1) / 100);
...
\\ v2: f_pi == 2 was a v1 observation; report it, require only |D_K| > 2^100 as in the search rule.
must(abs(D_K) > 2^100, "fundamental CM discriminant too small");
...
\\ v2: expected-cost model with negation map, sqrt(pi*q/4), as quoted in the v1 README (~149.3 bits).
rho_negmap_bits_q = log(sqrt(Pi * q / 4)) / log(2);
rho_negmap_bits_qt = log(sqrt(Pi * qt / 4)) / log(2);
...
emit(Str("q_bitlength=", logint(q, 2) + 1));
emit(Str("four_q_ge_2pow301=", 4 * q >= 2^301));
emit(Str("qt=", qt));
emit(Str("qt_isprime=", isprime(qt)));
emit(Str("qt_bitlength=", logint(qt, 2) + 1));
emit(Str("N=", N));
...
emit(Str("embedding_degree_q_cofactor_in_q_minus_1=", (q - 1) / k_q));
...
emit(Str("embedding_degree_qt_cofactor_in_qt_minus_1=", (qt - 1) / k_qt));
emit(Str("embedding_degree_qt_bitlength=", logint(k_qt, 2) + 1));
emit(Str("embedding_degree_qt_prime_divisor_witnesses=", W_qt));
emit(Str("safecurves_floor_k_ge_order_minus_1_over_100=", safecurves_floor_pass));
...
emit(Str("rho_negation_map_expected_bits_q=", rho_negmap_bits_q));
emit(Str("log2_qt=", log2_qt));
emit(Str("rho_security_bits_sqrt_qt=", rho_bits_qt));
emit(Str("floor_sqrt_qt=", sqrtint(qt)));
emit(Str("rho_negation_map_expected_bits_qt=", rho_negmap_bits_qt));
...
\\ v2: every order between Z[pi] and O_K is possible: conductor g | f_pi, discriminant g^2 * D_K.
emit(Str("possible_endomorphism_ring_conductors=", divisors(f_pi)));
emit(Str("possible_endomorphism_ring_discriminants=", vector(#divisors(f_pi), i, divisors(f_pi)[i]^2 * D_K)));
...
emit(Str("field_modulus_pseudo_mersenne_relation_pass=", 2^301 == p + 2^89 - 907));
emit("audit_pass=1");
