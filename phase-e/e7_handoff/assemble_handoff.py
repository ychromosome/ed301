...
FINAL_SEAL = "phase-e/E7_FINAL_HANDOFF_SOURCE_MANIFEST.sha256"
AUTHORIZATIONS = {
    "PHASE_E_E7_LEITER_UND_QUADRIERUNG_2026-09-10.md":
        "98de6fcddfbf679edda7cfd05eba07a08d779fd84bafd7ed0b9bc14ce02d0315",
    "PHASE_E_E7_VARIANTE_AFL_CLAUDE.diff":
        "ffc4f1650c79a8d764928ce8c252510d08c47b5714c233a721012501225df929",
    "PHASE_E_E7_MESSUNGEN_CLAUDE.txt":
...
def main():
    ...
    if (source_identity.get("branch") != "Testing" or source_identity.get("base_commit") != BASE_COMMIT
            or source_identity.get("source_manifest_sha256") != SOURCE_SHA or source_identity.get("file_count") != 1906):
        raise SystemExit("unexpected final source identity")
    index = {
        "format": "ed301-v2-phase-e-e7-final-index-v1", "evidence_root": str(root),
        ...
        "evidence_policy": "fresh E7 results; no E1-E6 or Gate-C/D PASS receipt transferred",
    }
    ...
    if correctness["test_counts"] != {"baseline-ed": 54, "baseline-x": 25,
            "previous-ed": 61, "previous-x": 54, "current-ed": 62, "current-x": 56,
            "current-ed-self-verify": 62, "current-x-taint-feature": 56}:
    ...
    # Retain the exact historical sources rebuilt by this new E7 correctness
    ...
    for raw in sorted(set(listed.split(b"\0")) - {b""}):
        ...
        if (path.is_symlink() or not path.is_file() or not name.startswith("phase-e/")
                or not (path.suffix == ".md" or name.startswith("phase-e/e7_handoff/"))):
...
def create_bundle(bundle, root, index_path, index):
    ...
    copy_member(index_path, bundle / "reports/E7_EVIDENCE_INDEX.json")
    copy_member(CHECKOUT / FINAL_SEAL, bundle / "reports/E7_FINAL_HANDOFF_SOURCE_MANIFEST.sha256")
