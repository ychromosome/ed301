...
def verify(bundle, expected):
    ...
    index = read_json(bundle / "reports/E7_EVIDENCE_INDEX.json")
    if (index["format"] != "ed301-v2-phase-e-e7-final-index-v1" or index["source_manifest_sha256"] != SOURCE_SHA
    ...
    final_rows = manifest_rows(bundle / "reports/E7_FINAL_HANDOFF_SOURCE_MANIFEST.sha256")
    if set(final_rows) != set(source_rows) | set(index["post_execution_files"]) | {"phase-e/E7_EVIDENCE_INDEX.json"}:
