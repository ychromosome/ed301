...
FINAL_SEAL = "phase-e/FINAL_HANDOFF_SOURCE_MANIFEST.sha256"
AUTHORIZATIONS = {
...
def main():
    ...
    if (source_identity.get("branch") != "Testing" or source_identity.get("base_commit") != BASE_COMMIT
            or source_identity.get("source_manifest_sha256") != SOURCE_SHA or source_identity.get("file_count") != 1888):
        raise SystemExit("unexpected final source identity")
    index = {
        "format": "ed301-v2-phase-e-final-index-v1", "evidence_root": str(root),
        ...
        "evidence_policy": "fresh Phase-E results; no Gate-C/D timing or benchmark result transferred",
    }
    ...
    if correctness["test_counts"] != {"baseline-ed": 54, "baseline-x": 25,
            "current-ed": 61, "current-x": 54, "current-ed-self-verify": 61, "current-x-taint-feature": 54}:
    ...
    for raw in sorted(set(listed.split(b"\0")) - {b""}):
        ...
        if (path.is_symlink() or not path.is_file() or not name.startswith("phase-e/")
                or not (path.suffix == ".md" or name.startswith("phase-e/handoff/"))):
...
def create_bundle(bundle, root, index_path, index):
    ...
    copy_member(index_path, bundle / "reports/PHASE_E_EVIDENCE_INDEX.json")
    copy_member(CHECKOUT / FINAL_SEAL, bundle / "reports/FINAL_HANDOFF_SOURCE_MANIFEST.sha256")
