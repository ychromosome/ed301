...
def verify(bundle, expected):
    ...
    index = read_json(bundle / "reports/E8_EVIDENCE_INDEX.json")
    if (index["format"] != "ed301-v2-phase-e-e8-final-index-v1" or index["source_manifest_sha256"] != SOURCE_SHA
    ...
    expected_supplements = set(FINAL_CORE) | set(EXTENDED_GATES) | {"benchmark-" + step for step in STEP_BENCHMARKS} | {
        ...
        "public-import-boundary", "codegen-driver-control", "provenance-control"}
    ...
    for name, record in index["supplementary"].items():
        ...
        if name == "provenance-control":
            identity = record["IDENTITY.json"]
            fixture = directory / "fixture/source"
            changed = identity["changed_member"]
            if (identity["original_member_sha256"] != source_rows.get(changed)
                    or digest(fixture / changed) != identity["synthetic_member_sha256"]
                    or [member for member, sha in source_rows.items()
                        if digest(fixture / member) != sha] != [changed]):
                raise SystemExit("provenance fixture differs beyond its controlled member")
    ...
    final_rows = manifest_rows(bundle / "reports/E8_FINAL_HANDOFF_SOURCE_MANIFEST.sha256")
    if set(final_rows) != set(source_rows) | set(index["post_execution_files"]) | {"phase-e/E8_EVIDENCE_INDEX.json"}:
