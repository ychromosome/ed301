...
"""Phase-E X301 taint: full corpus, or the explicit fixed-base/import subset."""
import argparse
import hashlib
import json
...
parser.add_argument("--fixed-base-only", action="store_true", help="only the seven successful public derivations and seven imports")
args = parser.parse_args()
WORK = Path(tempfile.mkdtemp(prefix="X301-v2_PHASE_E-taint_", dir=ROOT.parent))
...
if args.fixed_base_only:
    cases = [case for case in cases if case[1] in ("public", "import") and not case[5]]
    if len(cases) != 14:
        raise SystemExit("FAIL: fixed-base case inventory")
...
summary = {"status": "PASS", "cases": len(cases), "valgrind_runs": len(cases) * 2,
           "successful_public_and_import_cases": 14, "successful_shared_cases": 0 if args.fixed_base_only else 24,
           "ordinary_error_cases": 0 if args.fixed_base_only else 33, "weak_secret_cases": 0 if args.fixed_base_only else 64 * 3,
           "input_vbits_checked": True, "shared_output_keeps_taint": not args.fixed_base_only, "selection": "fixed-base-and-import-only" if args.fixed_base_only else "full-Gate-B-corpus",
