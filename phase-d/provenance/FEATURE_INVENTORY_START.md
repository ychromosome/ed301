...
Stand: 10. September 2026, vor Beginn der Rust-D1-Implementierung.
...
| ID | Öffentlicher Weg / Eigenschaft | Positiver Nachweis | Negativer Nachweis | v2-Stand vor Umsetzung |
|---|---|---|---|---|
| X01 | Raw Public / Shared / x301 | 7 Schlüssel, 4 DH-Paare, 24 Hauptkurven-/Twistauswertungen aus Gate B | 33 Fehlerfälle ohne Teilausgabe | nicht implementiert |
| X02 | Secretimport und wiederverwendbarer Besitzer | Originalbytes bleiben exportierbar; gültige Clamp-Aliase gleich | falsche Länge und alle 64 Nt-Aliase vor Arithmetik | nicht implementiert |
| X03 | Publicimport und wiederverwendbares Peerobjekt | strikt kanonische Hauptkurven-/Twist-u | Länge, u>=p, Bits 301..303; keine Maskierung/Reduktion | nicht implementiert |
| X04 | KeyGen mit fallibler Zufallsquelle | unabhängige 38-Byte-Ziehungen; deterministischer Testlieferant | nur WeakSecret wird erneut gezogen; RNG-/interner Fehler endet | nicht implementiert |
| X05 | Feste Montgomery-Leiter | 301 Runden, A24-minus, Gate-B-Kette bis 1000 | führende Nullbits im internen Test; Fehler vor Runde 1 bzw. erst nach voller Leiter klar getrennt | nicht implementiert |
| X06 | Fehlerreihenfolge | Secret wird vor Peer-u validiert | ungültiges Secret zusammen mit ungültigem u verdeckt Secretfehler nicht | nicht implementiert |
| X07 | Zeroisierende Secret-/Shared-Besitzer | Borrow-API, Normalreturn und Unwind | keine implizite Copy-/Debug-Ausgabe; Taint bleibt am Shared-Output | nicht implementiert |
| X08 | Kryptographische Implementierungsgates | no_std/Safe Rust, Oracle-/Grenztests, Taint, finales ELF, dudect | positive Instrumentkontrollen; unerlaubter Codegen wird erkannt | nicht getestet |
| X09 | Performance/Ressourcen | vergleichbare Rust- und EVP-Lanes, kalte/vorbereitete Schlüssel, Größen/Stack/RSS | keine erfundene Regressionsschwelle, keine Ebenenmischung | nicht gemessen |
