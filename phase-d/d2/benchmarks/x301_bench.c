...
/* Bound Phase-C XDH/KEM benchmark, with an explicit untimed module selector.
 * The measured loops are unchanged and one binary measures v1 and v2. */
...
int main(int argc, char **argv)
{
    ...
    if (argc != 8 || !parse_size(argv[6], MAX_KEY_BYTES, &key_bytes)
            || !parse_size(argv[7], 100000000U, &count) || count == 0) {
        fprintf(stderr, "usage: %s <keygen|derive-setup|derive-first|derive-second|derive-steady|kem-keygen|encaps|decaps> "
            "<algorithm> <properties|-> <module-dir|-> <provider> <key-bytes> <count>\n",
            argv[0]);
        return EXIT_FAILURE;
    }
    operation = argv[1];
    ...
    if (strcmp(module_directory, "-") != 0) {
        x301_provider = OSSL_PROVIDER_load(libctx, argv[5]);
