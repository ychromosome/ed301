...
# Parser and instruction rules from the bound Phase-C and D1 checkers.
# D2 applies them to actual linked provider DSOs, with separate profile policies.
TOOLS=$(CDPATH= cd -- "$(dirname -- "$0")" && pwd -P)

if [ "$#" -ne 4 ]; then
    printf 'usage: %s <ed|x> <provider-dso> <toolchain-marker> <evidence-directory>\n' \
        "$0" >&2
    exit 2
fi
PROFILE=$1
...
TOOLCHAIN=$3
EVIDENCE=$4
case "$PROFILE" in ed|x) ;; *) exit 2 ;; esac
...
/usr/bin/grep -Eq 'Type:[[:space:]]+DYN \(Shared object file\)$' \
    "$ELF_HEADER" || {
        echo "codegen input is not a shared-object DSO" >&2
        exit 1
    }
/usr/bin/grep -Eq \
...
printf 'PASS negative_control=provider-import-jcc-detected\n' | tee -a "$SUMMARY"
if /usr/bin/grep -Eq '(ed301_eddsa|x301_core)::.*(tests::|rem_wide|div3by2)' "$EVIDENCE/core.nm"; then
...
printf 'PASS provider_codegen profile=%s module_sha256=%s evidence=%s\n' \
