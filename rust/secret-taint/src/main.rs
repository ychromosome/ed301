...
fn apply_mode<const N: usize>(mode: Mode, secret: &mut [u8; N]) -> Result<(), String> {
    if mode == Mode::Tainted {
        mark_undefined(secret);
    }
    let expected = if mode == Mode::Tainted { 0xff } else { 0 };
    if validity_bits(secret)?.iter().any(|bits| *bits != expected) {
        return Err("secret input taint state is not the requested mode".into());
    }
    Ok(())
}
...
fn required_array<const N: usize>(name: &str) -> Result<[u8; N], String> {
    required_bytes(name)?
        .try_into()
        .map_err(|_| format!("{name} has the wrong hexadecimal length"))
}

fn required_bytes(name: &str) -> Result<Vec<u8>, String> {
    let value = env::var(name).map_err(|_| format!("missing {name}"))?;
    if value.len() % 2 != 0 {
        return Err(format!("{name} has an odd hexadecimal length"));
    }
