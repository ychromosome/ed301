//! Specialized five-limb arithmetic for `p = 2^301 - 2^89 + 907`.
//! Adapted from the v1 backend; bounds and expected values are newly derived.
//!
...
const FOLD_SUBTRAHEND: u64 = parameters::FOLD_SUBTRAHEND;
const MAX_SMALL_MULTIPLIER: u64 = parameters::MAX_SMALL_MULTIPLIER;
const _: () = assert!(parameters::SMALL_MULTIPLIER_BITS == 36);
...
impl Fe301 {
    ...
    /// `value < 2^36`, the canonical product is below 2^337. The lazy variant
    /// is below 2^338, giving high < 2^37 and high*907 < 2^47. One fold of
    /// `2^301 = 2^89 - 907`, underflow correction and one conditional
    /// subtraction suffice. The u64 argument is PUBLIC, not unrestricted.
    #[inline(always)]
    pub(crate) fn mul_small(self, value: u64) -> Self {
        assert!(
            value <= MAX_SMALL_MULTIPLIER,
            "public multiplier exceeds 36-bit bound"
        );
        Self(conditional_subtract_modulus_ct(
            reduce_small_product_unreduced(multiply_five_by_u64(self.0, value)),
        ))
    }
    ...
    #[inline(always)]
    pub(crate) const fn mul_small_const(self, value: u64) -> Self {
        assert!(
            value <= MAX_SMALL_MULTIPLIER,
            "public multiplier exceeds 36-bit bound"
        );
        self.mul_const(Self::from_u64(value))
    }

    pub(crate) const fn neg_const(self) -> Self {
        Self::ZERO.sub_const(self)
    }
...
/// with `high < 2^37`; its penalty fits one word (<2^47). The positive
/// part is below 2^301+2^126 < 2p. Adding p after an underflow keeps the result
/// non-negative and below `2p`.  Callers that need a canonical value apply one
...
fn reduce_small_product_unreduced(product: [u64; LIMBS + 1]) -> [u64; LIMBS] {
    ...
    // Add high * 2^89, split across radix-2^64 limbs 1 and 2.
    let add_low = high << parameters::FOLD_SHIFT;
    let add_high = high >> (64 - parameters::FOLD_SHIFT);
    ...
    // Subtract the proved single-word penalty and add p after underflow.
...
// 2p in little-endian radix 2^64: the offset of the lazy subtraction and the
// bound of every lazily reduced value.
const MODULUS_TIMES_TWO: [u64; LIMBS] = parameters::TWO_P_WORDS;
...
/// Sum or difference of two lazy values, in `[0, 4p)`.
///
/// It has no encoding or comparison API and is consumed by exactly one
/// product, square or tightening step.
#[derive(Clone, Copy)]
...
impl Fe301Lazy {
    ...
    /// Canonicalise to `[0, p)` with one conditional subtraction.
    #[inline(always)]
    pub(crate) fn canonical(self) -> Fe301 {
        Fe301(conditional_subtract_modulus_ct(self.0))
    }

    /// Form a bounded sum below `4p` without reducing it.
    #[inline(always)]
    ...
    /// Multiply two lazy values, retaining the `[0, 2p)` bound.
    #[inline(always)]
    pub(crate) fn mul(self, rhs: Self) -> Self {
        Self(reduce_wide_unreduced(multiply_wide(self.0, rhs.0)))
    }

    /// Square a lazy value, retaining the `[0, 2p)` bound.
    #[inline(always)]
    pub(crate) fn square(self) -> Self {
    ...
    pub(crate) fn mul_small(self, value: u64) -> Self {
        ...
        Self(reduce_small_product_unreduced(multiply_five_by_u64(
            self.0, value,
        )))
    }
...
impl Fe301LazyLinear {
    ...
    /// Bring a value below `4p` back below `2p` with one conditional
    /// subtraction of `2p`, for use as a subtrahend or as a further summand.
    #[inline(always)]
...
#[inline(always)]
const fn multiply_five_by_u64(value: [u64; LIMBS], multiplier: u64) -> [u64; LIMBS + 1] {
...
/// `2^301 = 2^89 - 907 (mod p)`.
///
/// The subtrahend-free formulation multiplies the high part by the positive
/// two-limb constant `K = 2^89 - 907` and accumulates it onto the low part,
/// so every carry chain is an addition with a fixed trip count. For operands
/// below `4p < 2^303`, hence a wide product below `2^606`, the high part of
/// the first fold is below `2^305` and fits five limbs, the first fold is
/// below `2^301 + 2^394 < 2^395` and fits seven limbs; the remaining high part
/// is below `2^94` and fits two limbs. The final value is below
/// `2^301 + 2^183 < 2p`, so one conditional subtraction canonicalises it.
#[inline(always)]
...
/// Low limb of the positive fold constant K from the approved modulus.
const FOLD_CONSTANT_LOW: u64 = parameters::FOLD_LOW;
/// High limb of the positive fold constant K from the approved modulus.
const FOLD_CONSTANT_HIGH: u64 = parameters::FOLD_HIGH;
...
mod tests {
    ...
    fn oracle_reduce_wide(wide: [u64; LIMBS * 2]) -> [u8; FIELD_BYTES] {
        ...
        // Canonical 2^301 mod p, generated independently from the Gate-A p.
        let shift_bytes = parameters::FOLD_ENCODING;
        let shift =
            Oracle::from_canonical_bytes(&shift_bytes).expect_copied("fold constant is canonical");
    ...
    fn specialized_arithmetic_matches_the_montgomery_oracle() {
        ...
        for _ in 0..10_000 {
            ...
            for multiplier in [
                0_u64,
                1,
                parameters::EDWARDS_D_MAGNITUDE,
                u32::MAX as u64,
                (u32::MAX as u64) + 1,
                parameters::EDWARDS_A,
    ...
    #[test]
    fn lazy_arithmetic_matches_the_montgomery_oracle_and_bounds() {
        ...
        fn assert_below_four_p(value: Fe301LazyLinear) {
            let (doubled, carry) = add_limbs(MODULUS_TIMES_TWO, MODULUS_TIMES_TWO);
            assert_eq!(carry, 0);
            let (_, borrow) = subtract_limbs(value.0, doubled);
            assert_eq!(borrow, 1, "loose value escaped the [0, 4p) bound");
        }

        // Extreme operands: the largest lazy value 2p - 1 (not reachable from
        // a canonical element, so constructed directly), its loose sum
        // 4p - 2, and their products and squares.
        let mut two_p_minus_one = MODULUS_TIMES_TWO;
        two_p_minus_one[0] -= 1;
        let lazy_max = Fe301Lazy(two_p_minus_one);
        let lazy_max_oracle = oracle(lazy_max.canonical());
        let loose_max = lazy_max.add_loose(lazy_max);
        assert_below_four_p(loose_max);
        let squared = loose_max.square();
        let product = loose_max.mul(loose_max);
        let mixed = loose_max.mul_tight(lazy_max);
        let tightened = loose_max.tighten();
        let small = lazy_max.mul_small(MAX_SMALL_MULTIPLIER);
        for value in [squared, product, mixed, tightened, small] {
            assert_below_two_p(value);
        }
        let doubled_oracle = lazy_max_oracle.add(lazy_max_oracle);
        assert_eq!(
            squared.canonical().to_canonical_bytes(),
            doubled_oracle.square().to_canonical_bytes()
        );
        assert_eq!(
            product.canonical().to_canonical_bytes(),
            doubled_oracle.mul(doubled_oracle).to_canonical_bytes()
        );
        assert_eq!(
            mixed.canonical().to_canonical_bytes(),
            doubled_oracle.mul(lazy_max_oracle).to_canonical_bytes()
        );
        assert_eq!(
            tightened.canonical().to_canonical_bytes(),
            doubled_oracle.to_canonical_bytes()
        );
        assert_eq!(
            small.canonical().to_canonical_bytes(),
            lazy_max_oracle
                .mul(Oracle::from_u64(MAX_SMALL_MULTIPLIER))
                .to_canonical_bytes()
        );
        assert_eq!(
            lazy_max
                ...
                .canonical()
                .to_canonical_bytes(),
            Fe301::ZERO.to_canonical_bytes()
        );

        // Random operands through every lazy operation the group formulas use.
        let mut state = 0x4c41_5a59_4544_3330_u64;
        for _ in 0..10_000 {
            let left = generated(&mut state);
            let right = generated(&mut state);
            let third = generated(&mut state);
            let fourth = generated(&mut state);
            let (left_oracle, right_oracle, third_oracle, fourth_oracle) =
                (oracle(left), oracle(right), oracle(third), oracle(fourth));
            ...
            let lazy_product = left_lazy.mul(right_lazy);
            let lazy_square = third_lazy.square();
            let lazy_small = fourth_lazy.mul_small(parameters::EDWARDS_A);
            let loose_sum = lazy_product.add_loose(lazy_square);
            let loose_difference = lazy_small.sub_loose(lazy_product);
            assert_below_four_p(loose_sum);
            assert_below_four_p(loose_difference);
            let square = loose_sum.square();
            let product = loose_sum.mul(loose_difference);
            let mixed = loose_difference.mul_tight(lazy_square);
            let tight = loose_sum.tighten();
            for value in [
                lazy_product,
                lazy_square,
                lazy_small,
            ...
            let product_oracle = left_oracle.mul(right_oracle);
            let square_oracle = third_oracle.square();
            let small_oracle = fourth_oracle.mul(Oracle::from_u64(parameters::EDWARDS_A));
            let sum_oracle = product_oracle.add(square_oracle);
            let difference_oracle = small_oracle.sub(product_oracle);
            assert_eq!(
                square.canonical().to_canonical_bytes(),
                sum_oracle.square().to_canonical_bytes()
            );
            assert_eq!(
                product.canonical().to_canonical_bytes(),
                sum_oracle.mul(difference_oracle).to_canonical_bytes()
            );
            assert_eq!(
                mixed.canonical().to_canonical_bytes(),
                difference_oracle.mul(square_oracle).to_canonical_bytes()
            );
            assert_eq!(
                tight.canonical().to_canonical_bytes(),
                sum_oracle.to_canonical_bytes()
            );
        }
    }
    ...
    fn full_lazy_domain_matches_independent_wide_oracle() {
        ...
        for _ in 0..100_000 {
            ...
            let multiplier = splitmix64(&mut state) & MAX_SMALL_MULTIPLIER;
            let sparse = multiply_five_by_u64(lazy, multiplier);
    ...
    #[test]
    fn small_multiplication_const_runtime_and_lazy_negation_agree() {
        let mut largest = MODULUS;
        largest[0] -= 1;
        let mut lazy_largest = MODULUS_TIMES_TWO;
        lazy_largest[0] -= 1;
        for value in [
            Fe301::ZERO,
            Fe301::ONE,
            ...
            for multiplier in [
                ...
                assert_eq!(
                    value.mul_small(multiplier).to_canonical_bytes(),
                    value.mul_small_const(multiplier).to_canonical_bytes()
                );
            }
            assert_eq!(
                value.neg().to_canonical_bytes(),
                value.neg_const().to_canonical_bytes()
            );
        }
        for value in [Fe301Lazy::from_fe301(Fe301::ZERO), Fe301Lazy(lazy_largest)] {
            let negated = value.neg();
            assert_eq!(subtract_limbs(negated.0, MODULUS_TIMES_TWO).1, 1);
            assert_eq!(
                negated.canonical().to_canonical_bytes(),
                value.canonical().neg().to_canonical_bytes()
            );
        }
    }
    ...
    fn small_multiplier_rejects_values_outside_the_proved_bound() {
        let _ = Fe301::ONE.mul_small(MAX_SMALL_MULTIPLIER + 1);
    }
