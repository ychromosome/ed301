...
use crate::{generated_parameters, parameters::FIELD_BYTES};

// (p + 1) / 4 from Gate A. Since p = 3 (mod 4), raising a
...
const SQRT_EXPONENT: U320 = U320::from_be_hex(generated_parameters::SQRT_EXPONENT_HEX);
...
const SQRT_RATIO_EXPONENT: U320 = U320::from_be_hex(generated_parameters::SQRT_RATIO_EXPONENT_HEX);

const_monty_params!(
    FieldModulus,
    U320,
    generated_parameters::FIELD_MODULUS_HEX,
    "The ED301-v2 prime field modulus"
);
const_monty_form!(
    MontgomeryFieldElement,
    FieldModulus,
    "An element of the ED301-v2 prime field in Montgomery form"
);

const MODULUS: U320 = U320::from_be_hex(generated_parameters::FIELD_MODULUS_HEX);
...
mod tests {
    use super::*;
    const BASE_U_BYTES: [u8; FIELD_BYTES] = generated_parameters::BASE_U_ENCODING;
    ...
    fn square_root_is_verified_and_canonical_parity_is_reported() {
        ...
        assert!(
            root.ct_eq(&FieldElement::from_canonical_words(
                generated_parameters::SQRT_25_WORDS
            ))
            .to_bool()
        );
        let seven = FieldElement::from_u64(7);
        assert!(
            seven
                ...
                .ct_eq(&FieldElement::from_canonical_words(
                    generated_parameters::SQRT_49_WORDS
                ))
                .to_bool()
        );
        assert!(
