//! Rust-owned key and one-shot signature contexts for `Ed301-EdDSA-v2`.
//!
...
/// `Ed301-EdDSA-v2` callback table for the C provider shim.
...
#[derive(Clone, Default)]
pub(crate) struct V2SignatureContext {
...
fn hit_panic_failpoint(name: &str) {
    match std::env::var("ED301_EDDSA_V2_PANIC_FAILPOINT") {
...
pub(crate) extern "C" fn key_new() -> *mut c_void {
    ffi_pointer(|| {
        ...
        match try_box_at("key_new", V2Key::default()) {
...
pub(crate) unsafe extern "C" fn key_free(key: *mut c_void) {
    let _ = catch_unwind(AssertUnwindSafe(|| {
        if !key.is_null() {
            ...
            drop(unsafe { Box::from_raw(key.cast::<V2Key>()) });
        }
    }));
}
...
pub(crate) unsafe extern "C" fn key_import(
    ...
    ffi_int(|| {
        ...
        let Some(key) = (unsafe { key.cast::<V2Key>().as_mut() }) else {
            return 0;
        };
...
pub(crate) unsafe extern "C" fn key_duplicate(
    ...
    ffi_pointer(|| {
        // SAFETY: The shim passes a live Rust-owned key object.
        let Some(source) = (unsafe { source.cast::<V2Key>().as_ref() }) else {
            return core::ptr::null_mut();
        };
        ...
        match try_box_at("key_duplicate", V2Key { private, public }) {
            Some(key) => Box::into_raw(key).cast(),
            None => core::ptr::null_mut(),
        }
    })
...
pub(crate) unsafe extern "C" fn key_has(
    ...
    ffi_int(|| {
        // SAFETY: The shim passes a live Rust-owned key object.
        let Some(key) = (unsafe { key.cast::<V2Key>().as_ref() }) else {
            return 0;
        };
...
pub(crate) unsafe extern "C" fn key_match(
    ...
    ffi_int(|| {
        ...
        let Some(first) = (unsafe { first.cast::<V2Key>().as_ref() }) else {
            return 0;
        };
        // SAFETY: Same contract as for first.
        let Some(second) = (unsafe { second.cast::<V2Key>().as_ref() }) else {
            return 0;
        };
...
pub(crate) extern "C" fn signature_new() -> *mut c_void {
    ffi_pointer(|| {
        ...
        match try_box_at("signature_new", V2SignatureContext::default()) {
...
pub(crate) unsafe extern "C" fn signature_free(context: *mut c_void) {
    let _ = catch_unwind(AssertUnwindSafe(|| {
        if !context.is_null() {
            ...
            drop(unsafe { Box::from_raw(context.cast::<V2SignatureContext>()) });
        }
    }));
}
...
pub(crate) unsafe extern "C" fn signature_reset(context: *mut c_void) {
    let _ = catch_unwind(AssertUnwindSafe(|| {
        ...
        let Some(context) = (unsafe { context.cast::<V2SignatureContext>().as_mut() }) else {
            return;
        };
...
pub(crate) unsafe extern "C" fn signature_get_context(
    ...
    ffi_int(|| {
        // SAFETY: The shim passes a live Rust-owned signature context.
        let Some(context) = (unsafe { context.cast::<V2SignatureContext>().as_ref() }) else {
            return 0;
        };
...
pub(crate) unsafe extern "C" fn signature_duplicate(source: *const c_void) -> *mut c_void {
    ffi_pointer(|| {
        // SAFETY: The shim passes a live Rust-owned signature context.
        let Some(source) = (unsafe { source.cast::<V2SignatureContext>().as_ref() }) else {
            return core::ptr::null_mut();
        };
...
mod tests {
    ...
    fn provider_key_and_signature_state_remain_send_and_sync() {
        ...
        assert_send_sync::<V2Key>();
        assert_send_sync::<V2SignatureContext>();
    }
    ...
    fn null_key_reinitialization_preserves_only_the_matching_operation() {
        ...
        let mut context = Box::new(V2SignatureContext {
    ...
    fn signature_verify_preserves_the_provider_tristate() {
        ...
        let uninitialized = V2SignatureContext::default();
        assert_eq!(
            unsafe {
                signature_verify(
                    (&uninitialized as *const V2SignatureContext).cast(),
                    message.as_ptr(),
                    message.len(),
                    signature.as_ptr(),
                    signature.len(),
    ...
    fn signature_contexts_share_prepared_key_state() {
        ...
        assert_eq!(
            unsafe {
                signature_sign_init(
                    (&mut sign_context as *mut V2SignatureContext).cast(),
                    (&key as *const V2Key).cast(),
                )
            },
            1
        );
        ...
        let duplicate =
            unsafe { signature_duplicate((&sign_context as *const V2SignatureContext).cast()) };
        assert!(!duplicate.is_null());
        assert_eq!(expanded.reference_count(), 4);
        unsafe { signature_free(duplicate) };
        assert_eq!(expanded.reference_count(), 3);
        unsafe { signature_reset((&mut sign_context as *mut V2SignatureContext).cast()) };
        assert_eq!(expanded.reference_count(), 2);

        let mut verify_context = V2SignatureContext::default();
        assert_eq!(
            unsafe {
                signature_verify_init(
                    (&mut verify_context as *mut V2SignatureContext).cast(),
                    (&key as *const V2Key).cast(),
                )
            },
            1
