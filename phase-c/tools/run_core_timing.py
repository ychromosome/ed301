...
work = Path(tempfile.mkdtemp(prefix="ED301-v2_core-timing_", dir=ROOT.parent))
...
sources = sorted(p for base in (RUST, ROOT / "phase-c/timing") for p in base.rglob("*") if p.is_file())
...
run(["/usr/bin/cargo", "build", "--manifest-path", ROOT / "phase-c/timing/Cargo.toml",
     "--locked", "--offline", "--release"], "build-adapter", build)
run(["/bin/sh", RUST / "scripts/check-profile-markers.sh", work / "markers",
     "crypto_bigint=on", "ed301_eddsa=on", "ed301_core_timing_adapter=on"], "profile")
adapter = work / "libed301_core_timing_adapter.so"
shutil.copyfile(work / "target/release" / adapter.name, adapter)
binary = work / "core_timing"
run(["/usr/bin/gcc", "-std=c11", "-O2", "-Wall", "-Wextra", "-Werror",
     ROOT / "phase-c/timing/core_timing.c", "-L" + str(work), "-Wl,-rpath," + str(work),
     "-led301_core_timing_adapter", "-lm", "-o", binary], "build-harness")
...
identity = {"scope": "core fixed/random synthetic seeds, empty context, 64-byte message",
...
summary = "\n".join(line for line in output.splitlines()
                    if line.startswith(("CORE_TIMING", "core_timing"))) + "\n"
...
print(f"PASS: core-only timing observation; artifact_directory={work}", flush=True)
