...
# Derived from v1 check-final-provider-codegen.sh, commit 5c688206a15f6ab88a50d53fe503665a302cec4d.
# Source SHA-256: 62bf67715330164573e56a7462d12336233d216a0ae8eacbc907875404990b1d.
# This gate checks the linked x86-64 core benchmark, not a provider or other platform.
if [ "$#" -ne 3 ]; then
...
extract_symbol ed301_benchmark::main "$EVIDENCE/negative-control.asm"
...
if /usr/bin/grep -Eq 'ed301_eddsa::scalar::.*(rem_wide|div3by2)' \
        "$EVIDENCE/core.nm"; then
    echo "FAIL test-oracle wide division reached the core binary" >&2
    exit 1
fi
...
/usr/bin/sha256sum "$MODULE" "$TOOLCHAIN" "$DUMP" "$SUMMARY" \
    >"$EVIDENCE/SHA256SUMS"
printf 'PASS final_core_codegen module_sha256=%s toolchain_sha256=%s evidence=%s\n' \
